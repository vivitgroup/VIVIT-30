...
import { db, apiKeys, auditLogs } from "@/lib/db";
...
export async function POST(req:NextRequest){
  ...
  const key=await db.transaction(async tx=>{const [created]=await tx.insert(apiKeys).values({workspaceId,userId,name,keyHash,keyPrefix,permissions}).returning({id:apiKeys.id,name:apiKeys.name});await tx.insert(auditLogs).values({workspaceId,userId,action:"api_key_created",entity:"api_keys",entityId:created.id,newValues:JSON.stringify({name,permissions,keyPrefix})});return created});
...
export async function DELETE(req:NextRequest){
  ...
  const rows=await db.transaction(async tx=>{const changed=await tx.update(apiKeys).set({isActive:false}).where(and(eq(apiKeys.id,id),eq(apiKeys.workspaceId,workspaceId),eq(apiKeys.isActive,true))).returning({id:apiKeys.id,name:apiKeys.name});if(changed.length)await tx.insert(auditLogs).values({workspaceId,userId,action:"api_key_revoked",entity:"api_keys",entityId:id,newValues:JSON.stringify({name:changed[0].name})});return changed});
  if(!rows.length)return NextResponse.json({error:"API key not found or already inactive"},{status:404});
