...
export async function POST(req:NextRequest){
 ...
 try{await db.transaction(async tx=>{
  ...
  const [freshTask]=await tx.select({id:creativeTasks.id,status:creativeTasks.status,clientId:creativeTasks.clientId}).from(creativeTasks).where(and(eq(creativeTasks.id,taskId),eq(creativeTasks.workspaceId,workspaceId),eq(creativeTasks.status,"REVIEW"),sql`${creativeTasks.id} in (select id from creative_tasks where workspace_id=${workspaceId} and archived_at is null)`)).limit(1);
 ...
 })}catch(error){if(error instanceof Error&&error.message==="TASK_STATE_CHANGED")return NextResponse.json({error:"Task review state changed. Refresh and try again."},{status:409});throw error}
