...
export async function POST(req:NextRequest){
 ...
 try{const workspaceId=await resolveVivitoWorkspaceForUser(userId);return NextResponse.json(await decideVivitoDirectEvent(eventId,decision as "approve"|"reject",String(b.reason||"Explicit confirmation"),role,userId,workspaceId),{headers:{"Cache-Control":"private, no-store"}})}catch(e:unknown){const status=e&&typeof e==="object"&&"status" in e?Number((e as {status?:unknown}).status||400):400;return NextResponse.json({error:e instanceof Error?e.message:String(e)},{status,headers:{"Cache-Control":"private, no-store"}})}
}
