...
export async function POST(req:NextRequest){
 ...
 const attachments=Array.isArray(body.attachments)?body.attachments.slice(0,5).map((x:Record<string,unknown>)=>({fileId:String(x?.fileId||"").slice(0,100),name:String(x?.name||"").slice(0,255),mimeType:String(x?.mimeType||"").slice(0,120)})).filter(x=>x.fileId):[];
