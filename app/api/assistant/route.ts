...
import {db,creativeTasks,clients,sql} from "@/lib/db";
...
export async function POST(req:NextRequest){
 ...
 else if(role==="CLIENT")clientRows=await db.select({id:clients.id,companyName:clients.companyName}).from(clients).where(and(eq(clients.isActive,true),eq(clients.userId,userId)));
 ...
 let answer="";
 if(/today|today's|النهارده|اليوم|انهاردة|انهارده/.test(q)) answer=todayTasks.length?(arabic?`عندك ${todayTasks.length} تاسك ديدلاين النهارده:\n${todayTasks.map(line).join("\n")}`:`${todayTasks.length} task(s) are due today:\n${todayTasks.map(line).join("\n")}`):(arabic?"مفيش تاسكات ديدلاينها النهارده في نطاق شغلك الحالي.":"No tasks are due today in your current scope.");
 else if(/overdue|late|متأخر|متاخر|فات/.test(q)) answer=overdue.length?(arabic?`في ${overdue.length} تاسك متأخرة:\n${overdue.slice(0,10).map(line).join("\n")}`:`There are ${overdue.length} overdue task(s):\n${overdue.slice(0,10).map(line).join("\n")}`):(arabic?"مفيش تاسكات متأخرة حاليًا.":"There are no overdue tasks right now.");
 else if(/pending|معلق|معلقة|بندنج|قيد الانتظار/.test(q)) answer=pending.length?(arabic?`عندك ${pending.length} تاسك Pending:\n${pending.slice(0,10).map(line).join("\n")}`:`You have ${pending.length} pending task(s):\n${pending.slice(0,10).map(line).join("\n")}`):(arabic?"مفيش Pending tasks في نطاقك الحالي.":"There are no pending tasks in your current scope.");
 else if(/deadline|dead line|ديدلاين|ميعاد|موعد|امتى|متي/.test(q)) answer=soon.length?(arabic?`أقرب الديدلاينز:\n${soon.map(line).join("\n")}`:`Upcoming deadlines:\n${soon.map(line).join("\n")}`):(arabic?"مفيش ديدلاينز قادمة مسجلة حاليًا.":"No upcoming deadlines are recorded right now.");
 else if(/task|tasks|تاسك|تاسكات|مهام|مهمة/.test(q)) answer=tasks.length?(arabic?`عندك ${tasks.length} تاسك نشطة في نطاقك. الأقرب:\n${soon.slice(0,6).map(line).join("\n")}`:`You have ${tasks.length} active task(s). Nearest deadlines:\n${soon.slice(0,6).map(line).join("\n")}`):(arabic?"مفيش تاسكات نشطة في نطاقك الحالي.":"There are no active tasks in your current scope.");
 else answer=arabic?`أقدر أجاوبك مباشرة من بيانات الـERP عن التأسكات، ديدلاينز النهارده، المتأخر، والـPending. العناصر المؤرشفة لا تدخل في الإجابات اليومية.`:`I can answer directly from ERP data about tasks, today's deadlines, overdue work and pending items. Archived work is excluded from daily answers.`;
