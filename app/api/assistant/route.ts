...
async function clientScope(role:string,userId:string):Promise<any[]>{
 let result:any;
 if(["SUPER_ADMIN","ACCOUNTANT"].includes(role))result=await db.execute(sql`select id,company_name,industry from clients where workspace_id=${W} and is_active=true order by company_name`);
 else if(role==="ACCOUNT_MANAGER")result=await db.execute(sql`select id,company_name,industry from clients where workspace_id=${W} and is_active=true and account_manager_id=${userId} order by company_name`);
 else if(role==="MEDIA_BUYER")result=await db.execute(sql`select id,company_name,industry from clients where workspace_id=${W} and is_active=true and media_buyer_id=${userId} order by company_name`);
 else if(role==="CLIENT")result=await db.execute(sql`select id,company_name,industry from clients where workspace_id=${W} and is_active=true and user_id=${userId} limit 1`);
 else return[];
 return rows(result);
}
...
async function trackingContext(role:string,ids:string[]):Promise<any[]>{if(!ids.length||!["SUPER_ADMIN","ACCOUNT_MANAGER","MEDIA_BUYER"].includes(role))return[];return rows(await db.execute(sql`select t.client_id,c.company_name,t.platform,t.pixel_status,t.capi_status,t.utm_status,t.landing_page_status,t.issues,t.checked_at from tracking_health t join clients c on c.id=t.client_id where c.workspace_id=${W} and c.is_active=true and t.client_id in (${idsSql(ids)}) order by t.checked_at desc limit 50`))}
async function clientHealthContext(role:string,ids:string[]):Promise<any[]>{if(!ids.length||!["SUPER_ADMIN","ACCOUNT_MANAGER","MEDIA_BUYER"].includes(role))return[];return rows(await db.execute(sql`select id,company_name,industry,health_score,performance_score,churn_risk,churn_probability,media_budget,target_leads from clients where workspace_id=${W} and is_active=true and id in (${idsSql(ids)}) order by health_score asc limit 60`))}
async function salesContext(role:string,userId:string):Promise<any[]>{if(!["SUPER_ADMIN","SALES"].includes(role))return[];const ownerFilter=role==="SALES"?sql`and l.sales_rep_id=${userId}`:sql``;return rows(await db.execute(sql`select l.id,l.company_name,l.stage,l.estimated_value,l.probability,l.next_follow_up,l.follow_up_count,l.industry,l.expected_close,l.updated_at,(select max(a.created_at) from sales_activities a where a.lead_id=l.id) last_activity_at from sales_leads l where l.workspace_id=${W} and l.archived_at is null ${ownerFilter} order by coalesce(l.next_follow_up,l.expected_close,l.updated_at) asc limit 100`))}
async function financeContext(role:string,ids:string[]):Promise<{billing:any[];expenses:any[]}>{if(!["SUPER_ADMIN","ACCOUNTANT"].includes(role))return{billing:[],expenses:[]};let billing:any[]=[];if(ids.length)billing=rows(await db.execute(sql`select c.company_name,p.amount_due,p.amount_paid,p.amount_remaining,p.payment_day,p.payment_status from client_payment_profiles p join clients c on c.id=p.client_id where c.workspace_id=${W} and c.is_active=true and p.client_id in (${idsSql(ids)}) order by p.amount_remaining desc limit 80`));const expenses=rows(await db.execute(sql`select category,coalesce(sum(amount),0) amount from company_expenses where workspace_id=${W} and date>=date_trunc('month',now()) group by category order by amount desc limit 20`));return{billing,expenses}}
async function actionStaff(role:string){if(!["SUPER_ADMIN","ACCOUNT_MANAGER"].includes(role))return[];return rows(await db.execute(sql`select id,name,role from users where workspace_id=${W} and is_active=true and role in ('CREATOR','ACCOUNT_MANAGER','MEDIA_BUYER') order by name limit 120`))}
