...
type ClientRow={id:unknown;company_name?:unknown;industry?:unknown};
...
type MediaSummaryTotals={spend:number;results:number;atc:number;purchases:number;revenue:number;previousSpend:number;previousResults:number;previousPurchases:number;previousRevenue:number};
...
async function salesContext(role:string,userId:string,workspaceId:string):Promise<SalesRow[]>{if(!["SUPER_ADMIN","SALES"].includes(role))return[];const ownerFilter=role==="SALES"?sql`and l.sales_rep_id=${userId}`:sql``;return rows<SalesRow>(await db.execute(sql`select l.id,l.company_name,l.stage,l.estimated_value,l.probability,l.next_follow_up,l.follow_up_count,l.industry,l.expected_close,l.updated_at,(select max(a.created_at) from sales_activities a where a.lead_id=l.id) last_activity_at from sales_leads l where l.workspace_id=${workspaceId} and l.archived_at is null ${ownerFilter} order by coalesce(l.next_follow_up,l.expected_close,l.updated_at) asc limit 100`))}
async function financeContext(role:string,ids:string[],workspaceId:string):Promise<{billing:FinanceBillingRow[];expenses:Record<string,unknown>[]}>{if(!["SUPER_ADMIN","ACCOUNTANT"].includes(role))return{billing:[],expenses:[]};let billing:FinanceBillingRow[]=[];if(ids.length)billing=rows<FinanceBillingRow>(await db.execute(sql`select c.company_name,p.amount_due,p.amount_paid,p.amount_remaining,p.payment_day,p.payment_status from client_payment_profiles p join clients c on c.id=p.client_id where c.workspace_id=${workspaceId} and c.is_active=true and p.client_id in (${idsSql(ids)}) order by p.amount_remaining desc limit 80`));const expenses=rows<Record<string,unknown>>(await db.execute(sql`select category,coalesce(sum(amount),0) amount from company_expenses where workspace_id=${workspaceId} and date>=date_trunc('month',now()) group by category order by amount desc limit 20`));return{billing,expenses}}
...
export async function POST(req:NextRequest){
 ...
 const attachments:Attachment[]=Array.isArray(body.attachments)?body.attachments.slice(0,5).map((x:Record<string,unknown>)=>({fileId:String(x?.fileId||"").slice(0,100),name:String(x?.name||"").slice(0,255),mimeType:String(x?.mimeType||"").slice(0,120)})).filter((x:Attachment)=>x.fileId):[];
 ...
 if(likelyCompetitiveChatIntent(question)){
  try{
   ...
   if(cp){const client=resolveAuthorizedClient(clients,cp.clientName);if(!client)return NextResponse.json({answer:isArabic(question)?"حدد اسم العميل/البراند اللي هنراقب المنافسين بتوعه بوضوح.":"Specify the client/brand whose competitors should be monitored.",mode:"competitive-clarification",intelligence:"VIVITO"},{headers:{"Cache-Control":"private, no-store"}});
    ...
    let added=0;for(const comp of cp.competitors){if(!comp.urls.length)continue;const w=rows<{id:unknown}>(await db.execute(sql`insert into competitor_watchlists(workspace_id,client_id,competitor_name,created_by) values(${workspaceId},${String(client.id)},${comp.name||"Competitor"},${userId}) returning id`))[0];for(const url of comp.urls){const platform=platformFromUrl(url);await db.execute(sql`insert into competitor_social_profiles(watchlist_id,platform,profile_url) values(${String(w.id)},${platform},${url}) on conflict do nothing`);added++}}
 ...
 if(likelyVivitoArtifactIntent(question)){
  try{
   const kind=requestedArtifactKind(question);let research:Awaited<ReturnType<typeof groundedVivitoResearch>>|null=null,visionText="";
 ...
 const canSeeFinance=["SUPER_ADMIN","ACCOUNTANT"].includes(role),context:AssistantContext={role,scope:{clientCount:clients.length,clientNames:clients.map(c=>c.company_name)},clients,operations:buildOperations(tasks),topTasks:tasks.slice(0,40),operationalMemory:memories.slice(0,40)};
