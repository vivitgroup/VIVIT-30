...
async function actionStaff(role:string,workspaceId:string):Promise<StaffRow[]>{if(!["SUPER_ADMIN","ACCOUNT_MANAGER"].includes(role))return[];return Array.from(await db.execute<StaffRow>(sql`select id,name,role from users where workspace_id=${workspaceId} and is_active=true and role in ('CREATOR','ACCOUNT_MANAGER','MEDIA_BUYER') order by name limit 120`))}
