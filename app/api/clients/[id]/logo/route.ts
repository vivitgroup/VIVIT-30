...
async function access(id:string){
  ...
  if(!workspaceId)return{error:NextResponse.json({error:"Workspace unavailable"},{status:403})};
...
export async function POST(req:NextRequest,{params}:{params:Promise<{id:string}>}){
  const{id}=await params,a=await access(id);if("error" in a)return a.error;
  if(!["SUPER_ADMIN","ACCOUNT_MANAGER","MEDIA_BUYER"].includes(a.role))return NextResponse.json({error:"Forbidden"},{status:403});
  const b=await req.json().catch(()=>({})),logo=String(b.logo||"").trim();
  if(logo.length>600000)return NextResponse.json({error:"Logo image is too large"},{status:413});
  if(logo&&!logo.startsWith("data:image/")&&!url(logo))return NextResponse.json({error:"Invalid logo image"},{status:400});
  ...
  await db.execute(sql`update clients set logo=${logo||null},facebook_url=${facebookUrl},instagram_url=${instagramUrl},website=${website},updated_at=now() where id=${id} and workspace_id=${a.workspaceId}`);
  const [primary]=await db.select({id:contacts.id}).from(contacts).where(and(eq(contacts.clientId,id),eq(contacts.isPrimary,true))).limit(1);
  if(primary)await db.update(contacts).set({whatsapp:whatsapp||null}).where(eq(contacts.id,primary.id));
  await db.insert(auditLogs).values({userId:a.userId,action:"client_profile_identity_updated",entity:"clients",entityId:id,newValues:JSON.stringify({hasLogo:Boolean(logo),facebookUrl,instagramUrl,website,hasWhatsapp:Boolean(whatsapp)})});
