...
export async function POST(req:NextRequest){
  ...
  const [client]=await db.insert(clients).values({companyName,industry:str(b.industry,100)||null,website:str(b.website,500)||null,monthlyRetainer:num(b.monthlyRetainer),mediaBudget:canSetupMarketing?num(b.mediaBudget):0,contractValue:num(b.contractValue),userId:portalUserId,accountManagerId:canSetupMarketing?(role==="ACCOUNT_MANAGER"?userId:str(b.accountManagerId,80)||null):null,mediaBuyerId:canSetupMarketing?(str(b.mediaBuyerId,80)||null):null,metaAdsLink:canSetupMarketing?(str(b.metaAdsLink)||null):null,tiktokAdsLink:canSetupMarketing?(str(b.tiktokAdsLink)||null):null,snapchatAdsLink:canSetupMarketing?(str(b.snapchatAdsLink)||null):null,googleAdsLink:canSetupMarketing?(str(b.googleAdsLink)||null):null,internalNotes:canSetupMarketing?(str(b.internalNotes,2000)||null):null,contractStart:date(b.contractStart),contractEnd:date(b.contractEnd)} as any).returning();
