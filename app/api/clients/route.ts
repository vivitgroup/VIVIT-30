...
export async function POST(req:NextRequest){
  ...
  const canSetupMarketing=role!=="ACCOUNTANT",portalUserId=canSetupMarketing?(str(b.portalUserId,80)||null):null;
  if(portalUserId){const [portalUser]=await db.select({id:users.id}).from(users).where(and(eq(users.id,portalUserId),eq(users.workspaceId,workspaceId),eq(users.role,"CLIENT"),eq(users.isActive,true),eq(users.approvalStatus,"APPROVED"))).limit(1);if(!portalUser)return NextResponse.json({error:"Choose a valid active approved client portal user."},{status:400});const existingLink=await db.select({id:clients.id}).from(clients).where(and(eq(clients.workspaceId,workspaceId),eq(clients.userId,portalUserId))).limit(1);if(existingLink[0])return NextResponse.json({error:"This portal user is already linked to another client."},{status:409});}
  const accountManagerId=canSetupMarketing?(role==="ACCOUNT_MANAGER"?userId:str(b.accountManagerId,80)||null):null,mediaBuyerId=canSetupMarketing?(str(b.mediaBuyerId,80)||null):null;
  if(accountManagerId){const [manager]=await db.select({id:users.id}).from(users).where(and(eq(users.id,accountManagerId),eq(users.workspaceId,workspaceId),eq(users.role,"ACCOUNT_MANAGER"),eq(users.isActive,true),eq(users.approvalStatus,"APPROVED"))).limit(1);if(!manager)return NextResponse.json({error:"Choose a valid active approved account manager."},{status:400});}
  if(mediaBuyerId){const [buyer]=await db.select({id:users.id}).from(users).where(and(eq(users.id,mediaBuyerId),eq(users.workspaceId,workspaceId),eq(users.role,"MEDIA_BUYER"),eq(users.isActive,true),eq(users.approvalStatus,"APPROVED"))).limit(1);if(!buyer)return NextResponse.json({error:"Choose a valid active approved media buyer."},{status:400});}
  const contractStart=date(b.contractStart),contractEnd=date(b.contractEnd);if(contractStart&&contractEnd&&contractEnd<contractStart)return NextResponse.json({error:"Contract end date must be on or after the start date."},{status:400});
  try{
    const clientId=await db.transaction(async tx=>{
      ...
      await tx.insert(auditLogs).values({workspaceId,userId,action:"client_created",entity:"Client",entityId:client.id,newValues:JSON.stringify({companyName,createdByRole:role})});
      return client.id;
    });
    return NextResponse.json({success:true,clientId},{status:201,headers:{"Cache-Control":"private, no-store"}});
  }catch(error){if(error instanceof Error&&error.message.startsWith("CLIENT_EXISTS:")){const clientId=error.message.slice("CLIENT_EXISTS:".length);return NextResponse.json({error:"A client with this company name already exists.",clientId},{status:409});}throw error;}
}
