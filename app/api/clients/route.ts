...
const sessionScope=async()=>{const session=await auth();if(!session?.user)return null;const workspaceId=str((session.user as any).workspaceId,160);if(!workspaceId)return null;return{session,workspaceId,role:String((session.user as any).role),userId:String((session.user as any).id)}};
export async function GET(){
  ...
  if(role==="CREATOR"){
    ...
    rows=await db.select({id:clients.id,companyName:clients.companyName}).from(clients).where(and(base,
      role==="ACCOUNT_MANAGER"?eq(clients.accountManagerId,userId):role==="MEDIA_BUYER"?eq(clients.mediaBuyerId,userId):role==="CLIENT"?eq(clients.userId,userId):undefined));
  }
...
export async function POST(req:NextRequest){
  ...
  await db.insert(auditLogs).values({workspaceId,userId,action:"client_created",entity:"Client",entityId:client.id,newValues:JSON.stringify({companyName})} as any);
