...
const parseCompetitors=(value:unknown):CompetitorInput[]|null=>{
 ...
 if(parsed.some(item=>item.name.length<2||![item.facebook,item.instagram,item.tiktok].every(validUrl)))return null;
 return parsed;
};
...
export async function POST(req:NextRequest){
  ...
  const competitors=parseCompetitors(b.competitors);if(!competitors)return NextResponse.json({error:"Add between 1 and 5 valid competitors. Each competitor needs a name and valid social links when provided."},{status:400});
  ...
  try{
   const clientId=await db.transaction(async tx=>{
    ...
    for(let index=0;index<competitors.length;index++){
      ...
      for(const [platform,profileUrl] of profiles)if(profileUrl)await tx.execute(sql`insert into competitor_social_profiles(id,watchlist_id,platform,profile_url,collection_mode,is_active,created_at) values(${crypto.randomUUID()}::uuid,${watchlistId}::uuid,${platform},${profileUrl},'PUBLIC_WEB',true,now())`);
    }
