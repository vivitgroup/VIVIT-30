...
export async function POST(req:NextRequest){
 ...
 let [existingConnection]=await db.select().from(adPlatformConnections).where(and(eq(adPlatformConnections.workspaceId,workspaceId),eq(adPlatformConnections.platform,platform),eq(adPlatformConnections.adAccountId,adAccountId))).limit(1);
