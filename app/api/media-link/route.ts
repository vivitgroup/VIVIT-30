...
export async function POST(req:NextRequest){
 ...
 let [connection]=await db.select().from(adPlatformConnections).where(and(eq(adPlatformConnections.platform,platform),eq(adPlatformConnections.adAccountId,adAccountId))).limit(1);
 if(connection&&(connection.clientId!==clientId||connection.workspaceId!==workspaceId))return NextResponse.json({error:"This ad account is already linked outside this client workspace. Reassign it explicitly as Super Admin instead of silently moving it."},{status:409});
 ...
 try{
  ...
  if(!connection){[connection]=await db.insert(adPlatformConnections).values({workspaceId,clientId,platform,adAccountId,accountName:String(body.accountName||`${platform} ${adAccountId}`).trim().slice(0,160),status:"CONNECTED",createdBy:userId}).onConflictDoUpdate({target:[adPlatformConnections.platform,adPlatformConnections.adAccountId],set:{workspaceId,clientId,status:"CONNECTED",syncError:null,updatedAt:new Date()}}).returning();}
  const [existingCampaign]=await db.select().from(adCampaigns).where(and(eq(adCampaigns.platform,platform),eq(adCampaigns.externalId,campaignId))).limit(1);if(existingCampaign&&(existingCampaign.clientId!==clientId||existingCampaign.workspaceId!==workspaceId))return NextResponse.json({error:"This campaign is already linked outside this client workspace."},{status:409});
  const [campaign]=await db.insert(adCampaigns).values({workspaceId,clientId,connectionId:connection.id,platform,externalId:campaignId,name:String(result.name||`${platform} Campaign ${campaignId}`).slice(0,160),status:String(result.status||"ACTIVE"),campaignUrl:platform==="META"?`https://adsmanager.facebook.com/adsmanager/manage/campaigns?act=${adAccountId}&selected_campaign_ids=${campaignId}`:`https://ads.tiktok.com/i18n/perf/campaign?aadvid=${adAccountId}&campaign_id=${campaignId}`,createdBy:userId,lastSyncAt:new Date()}).onConflictDoUpdate({target:[adCampaigns.platform,adCampaigns.externalId],set:{workspaceId,clientId,connectionId:connection.id,name:String(result.name||`${platform} Campaign ${campaignId}`).slice(0,160),status:String(result.status||"ACTIVE"),lastSyncAt:new Date(),updatedAt:new Date()}}).returning();
  await db.execute(sql`update ad_campaigns set archived_at=null,archived_by=null where id=${campaign.id} and workspace_id=${workspaceId}`);
  ...
  if(result.summary)await db.execute(sql`update ad_campaigns set reported_metrics=${JSON.stringify(result.summary)}::jsonb,reported_period_start=${startIso}::date,reported_period_end=${endIso}::date,reported_result_type=${result.summary.resultType||null},reported_result_label=${result.summary.resultLabel||null} where id=${campaign.id} and workspace_id=${workspaceId}`);
  await db.update(adPlatformConnections).set({lastSyncAt:new Date(),syncError:null,status:"CONNECTED",updatedAt:new Date()}).where(and(eq(adPlatformConnections.id,connection.id),eq(adPlatformConnections.workspaceId,workspaceId)));
  await db.insert(auditLogs).values({workspaceId,userId,action:"campaign_linked_by_id",entity:"ad_campaigns",entityId:campaign.id,newValues:JSON.stringify({platform,adAccountId,campaignId,days:result.days.length,metricSource:result.summary?.source||"PLATFORM"})});
  return NextResponse.json({success:true,campaign:{id:campaign.id,name:campaign.name,status:campaign.status,platform,adAccountId,campaignId},days:result.days.length,summary:result.summary||null},{headers:{"Cache-Control":"private, no-store"}});
 }catch(error:unknown){const message=String(error instanceof Error?error.message:"Campaign could not be loaded from the platform").slice(0,700);if(connection)await db.update(adPlatformConnections).set({syncError:message,status:"ERROR",updatedAt:new Date()}).where(and(eq(adPlatformConnections.id,connection.id),eq(adPlatformConnections.workspaceId,workspaceId)));return NextResponse.json({error:message},{status:400,headers:{"Cache-Control":"private, no-store"}});}
}
