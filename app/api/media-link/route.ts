...
export async function POST(req:NextRequest){
 ...
 try{
  const result=await syncCampaign({platform,campaignId,adAccountId:platform==="GOOGLE"?adAccountId.replace(/-/g,""):adAccountId,accessToken:accessToken||undefined,start:startIso,end:endIso});
  const saved=await db.transaction(async tx=>{
   ...
   for(const day of result.days){const date=`${day.date} 00:00:00`,spend=n(day.spend),results=n(day.results),purchases=n(day.purchases),impressions=n(day.impressions),clicks=n(day.clicks),revenue=n(day.revenue),reach=n(day.reach),frequency=n(day.frequency),addToCart=Math.max(0,Math.round(n(day.addToCart))),ctr=n(day.ctr)||(impressions?clicks/impressions*100:0),cpc=n(day.cpc)||(clicks?spend/clicks:0),cpm=n(day.cpm)||(impressions?spend/impressions*1000:0),costPerResult=n(day.costPerResult)||(results?spend/results:0),roas=spend?revenue/spend:0;await tx.execute(sql`delete from ad_performance_daily where campaign_id=${campaign.id} and date=${date}::timestamp and breakdown_type='TOTAL' and ad_id is null`);await tx.execute(sql`insert into ad_performance_daily(id,campaign_id,date,breakdown_type,breakdown_value,spend,impressions,reach,clicks,results,qualified_leads,purchases,add_to_cart,revenue,frequency,ctr,cpc,cpm,cpl,cost_per_result,cpa,roas) values(gen_random_uuid()::text,${campaign.id},${date}::timestamp,'TOTAL','ALL',${spend},${impressions},${reach},${clicks},${results},0,${purchases},${addToCart},${revenue},${frequency},${ctr},${cpc},${cpm},${costPerResult},${costPerResult},${purchases?spend/purchases:0},${roas})`)}
