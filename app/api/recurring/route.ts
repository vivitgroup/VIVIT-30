...
export async function POST(req: NextRequest) {
  ...
  for(const client of activeClients){
    ...
    const existing=await db.select({id:financeRecords.id}).from(financeRecords)
      .where(and(eq(financeRecords.workspaceId,WORKSPACE_ID),eq(financeRecords.clientId,client.id),eq(financeRecords.month,month),eq(financeRecords.year,year))).limit(1);
    if(existing.length)continue;
    const dueDate=new Date(year,month-1,15);
    await db.insert(financeRecords).values({
      workspaceId:WORKSPACE_ID,clientId:client.id,month,year,retainer,mediaBuyingFee:0,extraServices:0,
    ...
    for(const admin of admins){
      await db.insert(notifications).values({
        workspaceId:WORKSPACE_ID,userId:admin.id,type:"GENERAL",priority:"normal",
...
export async function GET(req: NextRequest) {
  ...
  const [activeClients,workspace]=await Promise.all([
    db.select({id:clients.id,companyName:clients.companyName,monthlyRetainer:clients.monthlyRetainer})
      .from(clients).where(and(eq(clients.workspaceId,WORKSPACE_ID),eq(clients.isActive,true))),
    db.select({currency:workspaces.currency}).from(workspaces).where(eq(workspaces.id,WORKSPACE_ID)).limit(1).then(r=>r[0])
  ]);
  ...
  for(const c of activeClients){
    ...
    const existing=await db.select({id:financeRecords.id}).from(financeRecords)
      .where(and(eq(financeRecords.workspaceId,WORKSPACE_ID),eq(financeRecords.clientId,c.id),eq(financeRecords.month,month),eq(financeRecords.year,year))).limit(1);
