...
export async function POST(req:NextRequest){
  try{
    ...
    const issued=await db.transaction(async tx=>{
      ...
      const cooldown=new Date(Date.now()-60_000),[recent]=await tx.select({id:emailVerificationCodes.id}).from(emailVerificationCodes).where(and(eq(emailVerificationCodes.email,normalizedEmail),gt(emailVerificationCodes.createdAt,cooldown))).limit(1);
    ...
    if(!issued)return NextResponse.json({error:"Please wait 60 seconds before requesting another code"},{status:429,headers:{"Retry-After":"60"}});
