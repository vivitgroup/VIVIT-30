...
export async function POST(req:NextRequest){
  try{
    ...
    if((await db.select({id:users.id}).from(users).where(eq(users.email,normalizedEmail))).length)return NextResponse.json({error:"Email already registered"},{status:409});
    if(requestedRole!=="CLIENT")return NextResponse.json({error:"Employee roles require Super Admin assignment"},{status:403});
    if(otpRequired){
      const[verification]=await db.select().from(emailVerificationCodes).where(and(eq(emailVerificationCodes.email,normalizedEmail),gt(emailVerificationCodes.expiresAt,new Date())));
    ...
    const workspaceId=workspaceRows[0].id,hashedPw=await bcrypt.hash(password,12),safeName=String(name).slice(0,120);
    await db.transaction(async tx=>{
      ...
      const[created]=await tx.insert(users).values({workspaceId,name:safeName,email:normalizedEmail,password:hashedPw,role:"CLIENT",requestedRole:"CLIENT",approvalStatus:"PENDING",approvalNote:approvalNote?.slice(0,500)||null,isWorkspaceOwner:false,isActive:false}).returning({id:users.id});
      const admins=await tx.select({id:users.id}).from(users).where(and(eq(users.workspaceId,workspaceId),eq(users.role,"SUPER_ADMIN"),eq(users.isActive,true)));
