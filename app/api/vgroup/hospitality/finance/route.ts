...
export async function GET(request:Request){
  try{
    ...
    if(propertyId&&!uuid.test(propertyId))return NextResponse.json({error:"invalid_property_id"},{status:400,headers:{"Cache-Control":"private, no-store"}});
    if(propertyId){
      ...
      if(!property)return NextResponse.json({error:"property_not_found"},{status:404,headers:{"Cache-Control":"private, no-store"}});
      ...
      return NextResponse.json({property:{id:property.id,name:property.name},statements:[],payouts:[],refunds:Array.from(refunds),deposits:Array.from(deposits),reservations:Array.from(reservations),propertyScoped:true},{headers:{"Cache-Control":"private, no-store"}});
    }
    ...
    return NextResponse.json({statements:Array.from(statements),payouts:Array.from(payouts),refunds:Array.from(refunds),deposits:Array.from(deposits),reservations:[],propertyScoped:false},{headers:{"Cache-Control":"private, no-store"}});
...
export async function POST(request:Request){
  try{
    ...
    if(!body.ownerId||!/^\d{4}-\d{2}-\d{2}$/.test(body.periodStart||"")||!/^\d{4}-\d{2}-\d{2}$/.test(body.periodEnd||""))return NextResponse.json({error:"invalid_statement_payload"},{status:400});
    return NextResponse.json({statement:await generateOwnerStatement(body.ownerId,body.periodStart!,body.periodEnd!)},{status:201});
  }catch(error){return apiErrorResponse(error)}
}
