...
export async function GET(){
  try{
    ...
    const [plans,subscriptions,invoices,slaIncidents]=await Promise.all([
      ...
      sql`select i.id::text,i.subscription_id::text,i.currency,i.amount,i.status,i.due_at,i.paid_at from tech.subscription_invoices i order by i.created_at desc limit 250`,
      sql`select i.id::text,i.project_id::text,i.subscription_id::text,i.title,i.status,i.opened_at,i.first_response_at,i.resolved_at from tech.sla_incidents i order by i.opened_at desc limit 100`,
    ]);
