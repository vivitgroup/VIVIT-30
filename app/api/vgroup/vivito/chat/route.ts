...
export async function POST(req:NextRequest){
  ...
  try{
    ...
    console.info("VIVITO run audit",{traceId,userId:session.userId,businessUnit:workspace,roles,provider:result.provider,modelId:result.modelId||null,attempted:result.attempted,fallbackChain:result.attempted,providerErrors:result.errors,latencyMs:result.latencyMs,researchRequested:wantsResearch,researchUsed:research.ok,evidenceCount:research.evidence.length,result:"answered",verification:"generation-returned"});
    return NextResponse.json({traceId,answer:result.text,modelId:result.modelId||null,provider:result.provider,fallbackChain:result.attempted,research:{requested:wantsResearch,configured:agentReachConfigured(),used:research.ok,evidenceCount:research.evidence.length,errorCode:research.ok?null:research.errorCode,latencyMs:research.latencyMs}},{headers:{"Cache-Control":"no-store"}});
  }catch(error:unknown){
