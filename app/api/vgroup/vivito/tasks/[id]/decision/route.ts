...
export async function POST(request:Request,{params}:{params:Promise<{id:string}>}){
  ...
  if(body.decision==="reject"){
    ...
    if(!rejected)return NextResponse.json({error:{code:"TASK_NOT_AWAITING_APPROVAL",message:"Task is not awaiting approval"}},{status:409,headers:NO_STORE});
  ...
  const [task]=await sql<{id:string;status:string;capability_key:string;payload_redacted:unknown}[]>`update vgroup.vivito_tasks set status='queued',approved_by=${session.userId}::uuid,approved_at=now(),updated_at=now() where id=${id}::uuid and status='waiting_approval' returning id::text,status,capability_key,payload_redacted`;
  if(!task)return NextResponse.json({error:{code:"TASK_NOT_AWAITING_APPROVAL",message:"Task is not awaiting approval"}},{status:409,headers:NO_STORE});
