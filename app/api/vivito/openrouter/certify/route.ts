...
export async function POST(req:NextRequest){
  ...
  const candidates:Candidate[]=[...openRouterModels.slice(0,MAX_OR).map(model=>({provider:"openrouter-free" as const,modelId:model.id,identity:canonicalModelIdentity(model.id)})),...groqModels.map(modelId=>({provider:"groq-free" as const,modelId,identity:canonicalModelIdentity(modelId)}))];
