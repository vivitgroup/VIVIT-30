...
type AuditData={op?:unknown;result?:{action?:unknown}};
const auditData=(v:unknown):AuditData=>{if(!v||typeof v!=="object"||Array.isArray(v))return{};const r=v as Record<string,unknown>,result=r.result;return{op:r.op,result:result&&typeof result==="object"&&!Array.isArray(result)?{action:(result as Record<string,unknown>).action}:undefined}};
export default async function VivitoOperationsPage(){
 ...
 const raw=Array.from(await db.execute(sql`select id,user_id,action,entity,entity_id,new_values,created_at from audit_logs where workspace_id=${workspaceId} and action like 'vivito_%' ${owner} order by created_at desc limit 120`)).map(r=>({id:String(r.id||""),action:String(r.action||""),entity_id:r.entity_id==null?null:String(r.entity_id),new_values:r.new_values,created_at:r.created_at instanceof Date?r.created_at:new Date(String(r.created_at||0))}));
 const events=raw.map(r=>{let parsed:unknown={};try{parsed=typeof r.new_values==="string"?JSON.parse(r.new_values):r.new_values||{}}catch{}const data=auditData(parsed),failed=r.action.includes("stopped")||r.action.includes("failed");return{...r,data,status:failed?"FAILED":"SUCCESS"}});
 return <div style={{display:"flex",flexDirection:"column",gap:18}}><div><h1 className="page-title">VIVITO Operator Control</h1><p className="page-subtitle">Proactive detection, safe execution, human approvals, escalation, outcome verification and learning.</p></div><DirectOperatorPanel/><div className="card"><div className="card-body"><h2 className="card-title">Unified audit trail</h2><div style={{overflowX:"auto",marginTop:14}}><table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}><thead><tr><th align="left">Time</th><th align="left">Action</th><th align="left">Operation</th><th align="left">Status</th><th align="left">Entity</th></tr></thead><tbody>{events.map(e=><tr key={e.id} style={{borderTop:"1px solid var(--card-border)"}}><td style={{padding:"10px 6px"}}>{new Date(e.created_at).toLocaleString()}</td><td>{String(e.action).replace(/_/g," ")}</td><td>{String(e.data?.op||e.data?.result?.action||"—")}</td><td>{e.status}</td><td>{e.entity_id||"—"}</td></tr>)}</tbody></table>{!events.length&&<p className="page-subtitle" style={{marginTop:14}}>No VIVITO audit events yet.</p>}</div></div></div></div>
}
