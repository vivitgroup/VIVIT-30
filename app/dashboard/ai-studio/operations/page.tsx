...
export default async function VivitoOperationsPage(){
 ...
 const events=raw.map(r=>{let data:unknown={};try{data=typeof r.new_values==="string"?JSON.parse(r.new_values):r.new_values||{}}catch{}const failed=String(r.action).includes("stopped")||String(r.action).includes("failed");return{...r,data,status:failed?"FAILED":"SUCCESS"}});
