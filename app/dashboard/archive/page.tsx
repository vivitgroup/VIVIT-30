...
export default function ArchivePage(){
 ...
 async function act(entity:"client"|"task"|"lead",row:Row,action:"restore"|"delete"){
  ...
  try{const r=await fetch("/api/lifecycle",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({entity,id:row.id,action})}),d=await r.json().catch(()=>({}));if(!r.ok)throw new Error(d.error||`Could not ${action} record`);await load()}catch(e){setError(e.message)}finally{setBusy("")}
 }
