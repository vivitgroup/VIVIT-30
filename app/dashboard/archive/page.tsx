...
export default function ArchivePage(){
 ...
 const section=(title:string,entity:"client"|"task"|"lead",rows:Row[])=><div className="card"><div className="card-header"><div><p className="card-title">{title}</p><p className="page-subtitle">{rows.length} archived</p></div><span className="badge badge-gray">{rows.length}</span></div><div className="card-body-flush">{rows.length===0?<div className="empty-state"><p>Nothing archived here.</p></div>:<div className="responsive-table"><table className="data-table"><thead><tr><th>Name</th><th>Archived</th><th>Actions</th></tr></thead><tbody>{rows.map(x=>{const key=`${entity}:${x.id}`,canRestore=entity==="client"?["SUPER_ADMIN","ACCOUNT_MANAGER"].includes(role):entity==="task"?["SUPER_ADMIN","ACCOUNT_MANAGER"].includes(role):(role==="SUPER_ADMIN"||role==="SALES"),canHardDelete=entity==="lead"?(role==="SUPER_ADMIN"||role==="SALES"):entity==="client"?["SUPER_ADMIN","ACCOUNT_MANAGER"].includes(role):role==="SUPER_ADMIN";return <tr key={x.id}><td style={{fontWeight:700}}>{x.name}</td><td>{x.archived_at?new Date(x.archived_at).toLocaleString():"—"}</td><td><div style={{display:"flex",gap:6,flexWrap:"wrap"}}>{canRestore&&<button className="btn btn-secondary btn-sm" disabled={busy===key} onClick={()=>act(entity,x,"restore")}>Restore</button>}{canHardDelete&&<button className="btn btn-danger btn-sm" disabled={busy===key} onClick={()=>act(entity,x,"delete")}>{busy===key?"Working…":"Delete permanently"}</button>}</div></td></tr>})}</tbody></table></div>}</div></div>;
