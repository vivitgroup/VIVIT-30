...
export default async function CalendarPage(){
 ...
 const [events,approvedTasks]=await Promise.all([
  db.select({id:calendarEvents.id,title:calendarEvents.title,date:calendarEvents.date,platform:calendarEvents.platform,caption:calendarEvents.caption,status:calendarEvents.status,clientId:calendarEvents.clientId}).from(calendarEvents).where(and(eventScope,gte(calendarEvents.date,start),lte(calendarEvents.date,end))).orderBy(calendarEvents.date),
