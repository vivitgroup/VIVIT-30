...
export default async function CalendarPage({searchParams}:{searchParams:Promise<{clientId?:string}>}){
 ...
 const role=(session.user as any).role as Role,userId=String((session.user as any).id),q=await searchParams;
 ...
 const taskScope=role===Role.CREATOR?and(eq(creativeTasks.assignedToId,userId),taskActive):(selectedClientId?and(eq(creativeTasks.clientId,selectedClientId),taskActive):(allowedClientIds.length?and(inArray(creativeTasks.clientId,allowedClientIds),taskActive):eq(creativeTasks.clientId,"__none__")));
