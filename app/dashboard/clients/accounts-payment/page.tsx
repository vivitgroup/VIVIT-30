...
export default async function AccountsPaymentPage(){
 ...
 const rows:any[]=Array.from(await db.execute(role===Role.ACCOUNT_MANAGER ? sql`
  ...
  where c.workspace_id='default' and c.account_manager_id=${userId}
 ...
 `}</style>
 <div className="pay-head"><div><h1 className="page-title">Accounts Payment</h1><p className="page-subtitle">{role===Role.ACCOUNT_MANAGER?"Collection status for your assigned clients":`Live collection status across ${rows.length} active client payment profiles`}</p></div><Link className="btn btn-secondary" href="/dashboard/clients" style={{textDecoration:"none"}}>Back to Clients</Link></div>
