...
export default async function TaskDetailGuard({children,params}:{children:ReactNode;params:Promise<{id:string}>}){
  ...
  const allowed=roles.includes(Role.SUPER_ADMIN)||
    (roles.includes(Role.ACCOUNT_MANAGER)&&task.account_manager_id===userId)||
    (roles.includes(Role.MEDIA_BUYER)&&task.media_buyer_id===userId)||
    (roles.includes(Role.CREATOR)&&task.assigned_to_id===userId)||
    (roles.includes(Role.CLIENT)&&task.client_user_id===userId);
