...
export default async function TaskDetailGuard({ children, params }: { children: ReactNode; params: Promise<{ id: string }> }) {
  ...
  const task = rows[0];
  if (!task) redirect(role === Role.CLIENT ? "/dashboard/portal" : "/dashboard/creative");
  if (task.client_active === false) redirect(role === Role.CLIENT ? "/dashboard/portal" : "/dashboard/creative");
