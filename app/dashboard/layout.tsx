...
import VivitoLocalFallbackRuntime from "@/components/assistant/VivitoLocalFallbackRuntime";
...
import "./live-ui-v10.css";
import "./live-ui-v11.css";
...
export default async function DashboardLayout({children}:{children:React.ReactNode}){
  ...
  return <div style={{display:"flex",minHeight:"100vh",width:"100%",maxWidth:"100vw",minWidth:0,overflowX:"hidden",background:"var(--bg-primary)"}}><a href="#dashboard-content" className="skip-to-content">Skip to main content</a><ExperienceRuntime role={role}/><VivitoOutputGuard/><VivitoLocalFallbackRuntime/><Sidebar role={role} userName={userName} canHrProvision={canHrProvision}/><div className="app-main-shell" id="app-main" style={{minWidth:0,width:"100%",maxWidth:"100%",overflowX:"hidden"}}><Header role={role} unreadCount={unreadCount}/><main id="dashboard-content" tabIndex={-1} style={{flex:1,padding:0,minWidth:0,maxWidth:"100%",overflowX:"hidden"}}><Suspense fallback={<PageSkeleton/>}><div className="app-content animate-fade-up" style={{minWidth:0,maxWidth:"100%"}}>{children}</div></Suspense></main></div><RealtimeNotifications/><KeyboardShortcutsModal/><TaskReferenceRuntime/><CompetitiveDeliveryApprovalRuntime role={String(role)}/><CampaignLifecycleRuntime role={String(role)}/><MobileNav role={role}/><DashboardLanguage/><ClientSocialLinkRuntime/>{showTaskReminders&&<TaskReminderWatcher/>}<SystemAssistant role={role}/><VivitoVoiceRuntime/><ClientLogoManager role={role}/><OperatingSystemLauncher role={role}/></div>;
}
