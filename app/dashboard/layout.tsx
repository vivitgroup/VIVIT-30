...
export default async function DashboardLayout({children}:{children:React.ReactNode}){
  ...
  return <div style={{display:"flex",minHeight:"100vh",background:"var(--bg-primary)"}}><ExperienceRuntime role={role}/><Sidebar role={role} userName={userName} canHrProvision={canHrProvision}/><div className="app-main-shell" id="app-main" dir="ltr" data-ui-language="en"><Header role={role} unreadCount={unreadCount}/><main style={{flex:1,padding:0}}><Suspense fallback={<PageSkeleton/>}><div className="app-content animate-fade-up">{children}</div></Suspense></main></div><RealtimeNotifications/><KeyboardShortcutsModal/><TaskReferenceRuntime/><CompetitiveDeliveryApprovalRuntime role={String(role)}/><CampaignLifecycleRuntime role={String(role)}/><MobileNav role={role}/><DashboardLanguage/><ClientSocialLinkRuntime/>{showTaskReminders&&<TaskReminderWatcher/>}<SystemAssistant role={role}/><ClientLogoManager role={role}/><OperatingSystemLauncher role={role}/></div>;
}
