...
export default async function DashboardLayout({children}:{children:React.ReactNode}){const session=await auth();if(!session?.user)redirect("/login");const role=(session.user as any).role??"CLIENT",userName=session.user.name??session.user.email??"User",userId=(session.user as any).id??"";const unreadRows=await db.select({value:count()}).from(notifications).where(and(eq(notifications.userId,userId),eq(notifications.isRead,false))).catch(()=>[]);const unreadCount=Number(unreadRows[0]?.value??0);return <div style={{display:"flex",minHeight:"100vh",background:"var(--bg-primary)"}}><Sidebar role={role} userName={userName}/><div className="app-main-shell" id="app-main"><WorkspaceHomeButton/><Header role={role} unreadCount={unreadCount}/><main style={{flex:1,padding:0}}><Suspense fallback={<PageSkeleton/>}><div className="app-content animate-fade-up">{children}</div></Suspense></main></div><WorkspaceCurrencyGuard/><LegacyUiGuard/><RealtimeNotifications/><KeyboardShortcutsModal/><MobileNav role={role}/><DashboardLanguage/><SystemAssistant role={role}/><ClientLogoManager role={role}/></div>}
