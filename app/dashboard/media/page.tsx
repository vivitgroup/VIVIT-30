    ...
    <div style={{display:"flex",flexDirection:"column",gap:"20px"}}>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:"12px"}}>
        <div>
          ...
          <p className="page-subtitle">{now.toLocaleDateString("en-US",{month:"long",year:"numeric"})} · {allClients.length} clients · MTD performance</p>
        </div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:"12px"}}>
        ...
        ].map(k=><div key={k.label} className={`kpi-card ${k.color}`}><div className="kpi-icon">{k.icon}</div><div className="kpi-label">{k.label}</div><div className="kpi-value" style={{fontSize:"1.4rem"}}>{k.value}</div></div>)}
      </div>

      <div className="card"><div className="card-header"><p className="card-title">Platform Performance — MTD</p></div><div className="card-body">
        {visiblePlatformMetrics.length===0?<p style={{textAlign:"center",color:"var(--text-muted)",padding:"24px"}}>No media data this month. Log metrics below.</p>:<div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(200px,1fr))",gap:"12px"}}>{visiblePlatformMetrics.map(p=>{const cfg=PLATFORMS[p.platform??"meta"]??PLATFORMS.meta,spend=Number(p.spend),leads=Number(p.leads),revenue=Number(p.revenue),roas=spend>0?(revenue/spend).toFixed(1):"—",cpl=leads>0?(spend/leads).toFixed(0):"—";return <div key={p.platform} style={{padding:"16px",borderRadius:"var(--radius-sm)",background:cfg.bg,border:`1px solid ${cfg.color}22`}}><div style={{display:"flex",alignItems:"center",gap:"8px",marginBottom:"12px"}}><span style={{fontSize:"20px"}}>{cfg.icon}</span><span style={{fontSize:"13px",fontWeight:700,color:cfg.color,textTransform:"capitalize"}}>{p.platform}</span></div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"8px"}}>{[{label:"Spend",value:fmt(spend)},{label:"Leads",value:String(leads)},{label:"ROAS",value:`${roas}×`},{label:"CPL",value:cpl==="—"?"—":`${cpl} EGP`}].map(m=><div key={m.label}><p style={{fontSize:"10px",color:"var(--text-muted)",fontWeight:600,textTransform:"uppercase",letterSpacing:"0.05em"}}>{m.label}</p><p style={{fontSize:"15px",fontWeight:800,color:"var(--text-primary)",fontFamily:"Sora,sans-serif"}}>{m.value}</p></div>)}</div></div>})}</div>}
      </div></div>

      {byClient.length>0&&<div className="card"><div className="card-header"><p className="card-title">Client Breakdown — MTD</p></div><div className="card-body-flush"><table className="data-table"><thead><tr><th>Client</th><th>Ad Spend</th><th>Leads</th><th>Revenue</th><th>ROAS</th><th>CPL</th><th>Budget Pacing</th></tr></thead><tbody>{byClient.map(c=>{const roasColor=c.roas>=3?"var(--green)":c.roas>=1.5?"var(--amber)":"var(--red)";return <tr key={c.id}><td><Link href={`/dashboard/clients/${c.id}`} style={{textDecoration:"none",fontWeight:700,fontSize:"13.5px",color:"var(--text-primary)"}}>{c.companyName}</Link></td><td style={{fontWeight:700}}>{fmt(c.spend)}</td><td style={{color:"var(--purple)",fontWeight:700}}>{c.leads.toLocaleString()}</td><td style={{color:"var(--green)",fontWeight:700}}>{fmt(c.revenue)}</td><td><span style={{fontSize:"13px",fontWeight:800,fontFamily:"Sora,sans-serif",color:roasColor,padding:"3px 10px",borderRadius:"12px",background:roasColor+"15"}}>{c.roas.toFixed(1)}×</span></td><td style={{color:"var(--text-secondary)",fontWeight:600}}>{c.cpl.toFixed(0)} EGP</td><td><div style={{display:"flex",alignItems:"center",gap:"8px",minWidth:"100px"}}><div className="progress-bar" style={{flex:1}}><div className="progress-fill" style={{width:`${Math.min(c.pacing,100)}%`,background:c.pacing>=100?"var(--red)":c.pacing>=80?"var(--amber)":"var(--green)"}}/></div><span style={{fontSize:"11px",fontWeight:700,color:c.pacing>=100?"var(--red)":c.pacing>=80?"var(--amber)":"var(--green)",flexShrink:0}}>{c.pacing}%</span></div></td></tr>})}</tbody></table></div></div>}

      <div className="card" style={{borderTop:"3px solid var(--vivit-blue)"}}><div className="card-header"><p className="card-title">📊 Log Campaign Metrics</p></div><div className="card-body"><form action={addMetrics}><div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:"12px",marginBottom:"12px"}}><div><label className="form-label">Client *</label><select name="clientId" required className="form-select"><option value="">Select client...</option>{allClients.map(c=><option key={c.id} value={c.id}>{c.companyName}</option>)}</select></div><div><label className="form-label">Platform *</label><select name="platform" required className="form-select">{Object.entries(PLATFORMS).map(([k,v])=><option key={k} value={k}>{v.icon} {k.charAt(0).toUpperCase()+k.slice(1)}</option>)}</select></div><div><label className="form-label">Monthly Budget (EGP)</label><input name="budget" type="number" min="0" placeholder="10,000" className="form-input"/></div><div><label className="form-label">Ad Spend (EGP) *</label><input name="adSpend" type="number" min="0" step="0.01" required placeholder="5,000" className="form-input"/></div><div><label className="form-label">Leads Generated</label><input name="leads" type="number" min="0" step="1" placeholder="250" className="form-input"/></div><div><label className="form-label">Revenue Attributed (EGP)</label><input name="revenue" type="number" min="0" step="0.01" placeholder="15,000" className="form-input"/></div></div><button type="submit" className="btn btn-primary">Log Metrics →</button></form></div></div>
    </div>
  );
}
