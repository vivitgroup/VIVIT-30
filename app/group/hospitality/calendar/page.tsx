...
export default async function HospitalityCalendarPage({searchParams}:{searchParams:Promise<{propertyId?:string}>}){
  ...
  return <main style={{minHeight:"100vh",padding:"32px 22px",fontFamily:"Inter,system-ui,sans-serif"}}><section style={{maxWidth:1240,margin:"0 auto"}}>
    <div style={{display:"flex",justifyContent:"space-between",gap:16,flexWrap:"wrap",alignItems:"start"}}><div><Link href={backHref} style={{fontWeight:900,textDecoration:"none"}}>← {propertyId?`${propertyName} dashboard`:"Hospitality"}</Link><div style={{fontSize:12,letterSpacing:".18em",fontWeight:900,color:"#D6AD5B",marginTop:30}}>HOSPITALITY CALENDAR</div><h1 style={{fontSize:"clamp(38px,6vw,68px)",letterSpacing:"-.055em",margin:"8px 0"}}>{propertyId?`${propertyName} · Airbnb calendar`:"Airbnb availability calendar"}</h1><p style={{color:"#C7B894",lineHeight:1.7,maxWidth:800}}>{propertyId?`Property context is locked to ${propertyName}. Only this apartment's Airbnb connection and availability blocks are shown.`:"Real Airbnb iCal synchronization across all connected VIVIT Hospitality properties. No fake availability and no manual copy of reservations."}</p></div>{canSync?(propertyId?syncable.map(channel=><AirbnbSyncButton key={channel.id} channelId={channel.id}/>):<AirbnbSyncAllButton/>):null}</div>
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(180px,1fr))",gap:12,margin:"26px 0"}}>{[["Airbnb listings",Array.from(channels).length],["Connected",connected],["Upcoming blocks",Array.from(blocks).length]].map(([label,value])=><article key={String(label)} style={{padding:18,borderRadius:18,border:"1px solid rgba(214,173,91,.28)"}}><div style={{fontSize:11,color:"#C7B894",fontWeight:900,textTransform:"uppercase"}}>{label}</div><strong style={{display:"block",fontSize:27,marginTop:8}}>{value}</strong></article>)}</div>
    <article style={{padding:22,borderRadius:24,border:"1px solid rgba(214,173,91,.28)",marginBottom:20}}><div style={{display:"flex",justifyContent:"space-between",gap:12,alignItems:"baseline",flexWrap:"wrap"}}><h2 style={{marginTop:0}}>Airbnb connections</h2><span style={{fontSize:12,color:"#C7B894"}}>Calendar integration: Airbnb iCal</span></div>{Array.from(channels).length===0?<p style={{color:"#C7B894"}}>No Airbnb calendar feed is configured for this context.</p>:<div style={{display:"grid",gap:12}}>{Array.from(channels).map(channel=><div key={channel.id} style={{display:"flex",justifyContent:"space-between",gap:16,flexWrap:"wrap",padding:"14px 0",borderBottom:"1px solid rgba(214,173,91,.12)"}}><div><strong>{channel.property_name}</strong><div style={{fontSize:12,color:"#C7B894",marginTop:5}}><a href={`https://www.airbnb.com/h/${channel.external_listing_id}`} target="_blank" rel="noreferrer">airbnb.com/h/{channel.external_listing_id}</a> · {channel.status}{channel.last_sync_at?` · Last sync ${channel.last_sync_at}`:" · Never synced"}</div>{channel.last_error&&<div style={{fontSize:12,color:"#FFB8B8",marginTop:5}}>{channel.last_error}</div>}</div>{canSync&&channel.status!=="disabled"&&channel.has_feed?<AirbnbSyncButton channelId={channel.id}/>:null}</div>)}</div>}</article>
    <article style={{padding:22,borderRadius:24,border:"1px solid rgba(214,173,91,.28)"}}><h2 style={{marginTop:0}}>Upcoming availability blocks</h2>{Array.from(blocks).length===0?<p style={{color:"#C7B894"}}>No upcoming Airbnb blocks yet.</p>:<div style={{display:"grid",gap:10}}>{Array.from(blocks).map(block=><div key={block.id} style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(160px,1fr))",gap:12,padding:"13px 0",borderBottom:"1px solid rgba(214,173,91,.12)",alignItems:"center"}}><strong>{block.property_name}</strong><span>{block.summary||"Unavailable"}</span><span style={{color:"#C7B894"}}>{fmt(block.starts_on)} → {fmt(block.ends_on)}</span></div>)}</div>}</article>
  </section></main>;
}
