...
type PropertyOption={id:string;name:string;max_guests:number;commission_percent:number|string|null;has_active_contract:boolean};
...
export default async function Page({searchParams}:{searchParams:Promise<{propertyId?:string}>}){
  ...
  const properties=await sql<PropertyOption[]>`
    select p.id::text,p.name,p.max_guests,
      c.commission_percent,
      (c.id is not null) has_active_contract
    from hospitality.properties p
    left join lateral (
      select id,commission_percent from hospitality.contracts c
      where c.property_id=p.id and c.archived_at is null and c.status='active'
        and c.starts_at<=current_date and (c.ends_at is null or c.ends_at>=current_date)
      order by c.starts_at desc,c.version desc limit 1
    ) c on true
    where p.archived_at is null and p.status='active'
    order by p.name`;
  const formProperties=Array.from(properties).map(p=>({id:p.id,name:p.name,maxGuests:Number(p.max_guests||1),commissionPercent:p.commission_percent==null?null:Number(p.commission_percent),hasActiveContract:Boolean(p.has_active_contract)}));
