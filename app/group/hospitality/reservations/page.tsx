...
export default async function Page({searchParams}:{searchParams:Promise<{propertyId?:string}>}){
  ...
  return <>
    <WorkspacePage tone="hospitality" eyebrow="HOSPITALITY OPERATIONS" title={propertyName?`${propertyName} · Reservations & Calendar`:"Reservations & Calendar"} description={propertyName?`Property context is locked to ${propertyName}. VIVIT reservations and Airbnb iCal availability are shown together without inventing guest or finance data from iCal.`:"Central reservation workspace with channel-aware availability, booking integrity and no-overlap protection."} sections={[{title:"Calendar",body:`${blocks.length} live Airbnb/channel availability blocks`},{title:"Reservations",body:`${rows.length} VIVIT reservation records in this view`},{title:"Channel Sync",body:"Airbnb iCal availability is propagated separately from trusted reservation records"},{title:"Booking Integrity",body:"Database-level prevention of double booking and invalid date ranges"}]}/>
    <section style={{maxWidth:1180,margin:"-40px auto 60px",padding:"0 20px",color:"#f7f1e3"}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(320px,1fr))",gap:18,marginTop:20}}>
        <section><h2>VIVIT reservations</h2><div style={{display:"grid",gap:10}}>{Array.from(rows).map(r=><article key={r.id} style={{padding:15,border:"1px solid #4a3a1c",borderRadius:14,display:"flex",justifyContent:"space-between",gap:14,flexWrap:"wrap"}}><div><b>{r.guest_name}</b><div style={{fontSize:12,color:"#b7aa8b"}}>{r.property_name} · {String(r.check_in)} → {String(r.check_out)}</div></div><div style={{fontSize:12,fontWeight:900}}>{r.status} · Net {Number(r.net_owner_amount||0).toFixed(2)}</div></article>)}{rows.length===0?<p style={{color:"#b7aa8b"}}>No VIVIT reservation records yet.</p>:null}</div></section>
