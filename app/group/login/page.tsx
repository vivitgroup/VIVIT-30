...
export default function GroupLogin(){
  ...
  useEffect(()=>{
    ...
    fetch(`/api/vgroup/auth/session?workspace=${encodeURIComponent(workspace)}`,{cache:"no-store"})
      .then(response=>{if(active&&response.ok)window.location.replace(`/group/enter/${workspace}`)})
      .catch(()=>undefined)
  ...
  return <main style={{minHeight:'100vh',display:'grid',placeItems:'center',padding:24,background:'radial-gradient(circle at 75% 20%,rgba(46,168,255,.14),transparent 30%),radial-gradient(circle at 20% 80%,rgba(178,138,59,.12),transparent 28%),#080b10',color:'#f8fafc',fontFamily:'Inter,system-ui,sans-serif'}}><form onSubmit={submit} style={{width:'min(430px,100%)',padding:30,borderRadius:28,border:'1px solid #263244',background:'rgba(13,18,27,.92)',boxShadow:'0 34px 90px rgba(0,0,0,.42)'}}><div style={{fontSize:11,fontWeight:900,letterSpacing:'.18em',color:'#8da2ba'}}>WORKSPACE ACCESS</div><h1 style={{fontSize:36,letterSpacing:'-.045em',margin:'10px 0 8px'}}>VIVIT Workspace</h1><p style={{color:'#91a0b3',lineHeight:1.6,margin:'0 0 28px'}}>{checking?'Checking your active VIVIT session…':'Sign in to the workspace you selected. Your selected destination is preserved and access is checked against live permissions.'}</p><label style={{display:'grid',gap:8,fontSize:12,fontWeight:800,marginBottom:16}}>EMAIL<input name="email" type="email" autoComplete="email" required disabled={checking} style={{height:48,borderRadius:14,border:'1px solid #2c3a4f',background:'#0c121b',color:'#fff',padding:'0 14px',fontSize:15}}/></label><label style={{display:'grid',gap:8,fontSize:12,fontWeight:800,marginBottom:18}}>PASSWORD<input name="password" type="password" autoComplete="current-password" required disabled={checking} style={{height:48,borderRadius:14,border:'1px solid #2c3a4f',background:'#0c121b',color:'#fff',padding:'0 14px',fontSize:15}}/></label>{error&&<div style={{padding:12,borderRadius:12,background:'rgba(220,38,38,.15)',border:'1px solid rgba(248,113,113,.35)',color:'#fecaca',fontSize:13,marginBottom:16}}>{error}</div>}<button disabled={loading||checking} style={{width:'100%',height:50,border:0,borderRadius:14,background:'linear-gradient(90deg,#b9964f,#49b8ff)',color:'#071018',fontWeight:950,fontSize:14,cursor:loading||checking?'wait':'pointer'}}>{checking?'Checking session…':loading?'Signing in…':'Enter selected workspace'}</button><div style={{marginTop:18,textAlign:'center'}}><Link href="/" style={{fontSize:12,color:'#8da2ba',textDecoration:'none'}}>← Choose another workspace</Link></div></form></main>;
}