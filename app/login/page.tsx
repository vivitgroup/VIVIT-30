  ...
  return (
    <div style={{minHeight:"100vh",display:"flex",background:"#F8FAFF",fontFamily:"'Plus Jakarta Sans',system-ui,sans-serif"}}>
      ...
      <div className="login-form-panel" style={{width:"480px",display:"flex",flexDirection:"column",justifyContent:"center",padding:"48px",background:"#fff",boxShadow:"-8px 0 40px rgba(0,0,0,0.08)"}}>
        <div style={{marginBottom:"32px"}}><h2 style={{fontSize:"1.6rem",fontWeight:800,color:"#0F172A",fontFamily:"Sora,sans-serif",letterSpacing:"-0.02em",marginBottom:"6px"}}>Sign in</h2><p style={{color:"#64748B",fontSize:"14px"}}>Welcome back to VIVIT ERP</p></div>
        <form onSubmit={login} style={{display:"flex",flexDirection:"column",gap:"16px"}}>
          {error && <div style={{background:"#FEF2F2",border:"1px solid #FECACA",borderRadius:"8px",padding:"10px 14px",color:"#DC2626",fontSize:"13px",fontWeight:500}}>⚠️ {error}</div>}
          <div><label style={{display:"block",fontSize:"12px",fontWeight:700,color:"#475569",marginBottom:"6px",textTransform:"uppercase",letterSpacing:"0.05em"}}>Email Address</label><input type="email" value={email} onChange={e=>setEmail(e.target.value)} required placeholder="you@vivitgroup.com" style={{width:"100%",padding:"11px 14px",border:"1.5px solid #E2E8F0",borderRadius:"8px",fontSize:"14px",color:"#0F172A",outline:"none",transition:"all 0.15s",fontFamily:"inherit",background:"#F8FAFF"}} onFocus={e=>e.target.style.borderColor="#C52A31"} onBlur={e=>e.target.style.borderColor="#E2E8F0"}/></div>
          <div><label style={{display:"block",fontSize:"12px",fontWeight:700,color:"#475569",marginBottom:"6px",textTransform:"uppercase",letterSpacing:"0.05em"}}>Password</label><input type="password" value={password} onChange={e=>setPassword(e.target.value)} required placeholder="••••••••" style={{width:"100%",padding:"11px 14px",border:"1.5px solid #E2E8F0",borderRadius:"8px",fontSize:"14px",color:"#0F172A",outline:"none",transition:"all 0.15s",fontFamily:"inherit",background:"#F8FAFF"}} onFocus={e=>e.target.style.borderColor="#C52A31"} onBlur={e=>e.target.style.borderColor="#E2E8F0"}/><div style={{textAlign:"right",marginTop:"7px"}}><Link href="/forgot-password" style={{fontSize:"12px",color:"#244D87",fontWeight:700}}>Forgot password?</Link></div></div>
          <button type="submit" disabled={loading} style={{padding:"12px",background:"linear-gradient(135deg,#9F1D25,#C52A31,#EFB324)",color:"#fff",border:"none",borderRadius:"10px",fontSize:"14px",fontWeight:700,cursor:"pointer",transition:"all 0.2s",boxShadow:"0 4px 16px rgba(33,150,243,0.35)",fontFamily:"inherit",marginTop:"4px",opacity:loading?0.7:1}}>{loading ? "Signing in..." : "Sign in to VIVIT ERP →"}</button>
        </form>
        <p style={{marginTop:"14px",textAlign:"center",fontSize:"13px",color:"#64748B"}}>Need an account? <Link href="/signup" style={{color:"#C52A31",fontWeight:800}}>Request access</Link></p>
        <div style={{marginTop:"32px",padding:"16px",background:"#F0F9FF",borderRadius:"10px",border:"1px solid #BAE6FD"}}><p style={{fontSize:"12px",color:"#0369A1",fontWeight:600,marginBottom:"4px"}}>🔒 Enterprise Security</p><p style={{fontSize:"11.5px",color:"#0284C7"}}>Secure sessions · bcrypt passwords · RBAC · audit logging</p></div>
      </div>
    </div>
  );
}
