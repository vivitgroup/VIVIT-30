...
export default function SignupPage() {
  ...
  useEffect(() => {
    fetch("/api/signup/otp")
      .then((response) => response.json())
      .then((data) => setOtpRequired(Boolean(data.configured)))
  ...
  return <main className="access-layout" style={{ minHeight: "100vh", display: "grid", gridTemplateColumns: "minmax(300px,.85fr) minmax(420px,1.15fr)", background: "#F8F5ED", fontFamily: "Inter,system-ui,sans-serif" }}>
    ...
    <section style={{ padding: "48px 7vw", display: "flex", alignItems: "center", justifyContent: "center" }}><div style={{ width: "100%", maxWidth: 650 }}>
      {sent ? <div style={{ background: "white", border: "1px solid #E9DFC9", borderRadius: 24, padding: 48, boxShadow: "0 20px 70px #251b1020", textAlign: "center" }}><div style={{ width: 72, height: 72, borderRadius: "50%", background: "#EAF3EC", display: "grid", placeItems: "center", fontSize: 34, margin: "0 auto 20px" }}>✓</div><h1 style={{ fontSize: 30, color: "#201F20", marginBottom: 12 }}>Request sent</h1><p style={{ color: "#6A6258", lineHeight: 1.7 }}>Your account is pending Super Admin approval. You will be able to sign in after it is approved.</p><Link href="/login" style={{ display: "inline-block", marginTop: 26, padding: "13px 24px", borderRadius: 12, background: "#C52A31", color: "white", textDecoration: "none", fontWeight: 800 }}>Back to sign in</Link></div> :
        <><p style={{ fontSize: 13, fontWeight: 800, letterSpacing: 1.5, color: "#C52A31" }}>CLIENT ACCESS REQUEST</p><h2 style={{ fontSize: 36, color: "#171717", margin: "8px 0", fontWeight: 900 }}>Create your account</h2><p style={{ color: "#70685F", marginBottom: 28 }}>Verify your email and request access to your company portal.</p>
          <form onSubmit={submit} style={{ display: "grid", gap: 16 }}>{error && <div role="alert" style={{ padding: "12px 14px", borderRadius: 10, background: "#FFF0F0", border: "1px solid #F0B8BB", color: "#A51F27" }}>{error}</div>}
            <label style={label}>FULL NAME<input className="vivit-input" required maxLength={120} autoComplete="name" value={form.name} onChange={(event) => update("name", event.target.value)} placeholder="Your full name" /></label>
            <label style={label}>EMAIL ADDRESS<div className="grid grid-cols-1 sm:grid-cols-[minmax(0,1fr)_auto] gap-2"><input className="vivit-input" type="email" required maxLength={254} autoComplete="email" value={form.email} onChange={(event) => { update("email", event.target.value); setOtpSent(false); }} placeholder="you@company.com" />{otpRequired && <button type="button" onClick={sendOtp} disabled={loading || !form.email} className="auth-secondary">{otpSent ? "Resend code" : "Send OTP"}</button>}</div></label>
            {otpStatusLoaded && !otpRequired && <div style={{ padding: "11px 13px", borderRadius: 12, background: "#F8F1DF", color: "#75531B", fontSize: 12, fontWeight: 700 }}>Email OTP is not configured. You can still submit a client access request; the Super Admin must review it before activation.</div>}
            {otpRequired && otpSent && <label style={label}>6-DIGIT VERIFICATION CODE<input className="vivit-input otp-input" inputMode="numeric" autoComplete="one-time-code" pattern="[0-9]{6}" maxLength={6} required value={form.otp} onChange={(event) => update("otp", event.target.value.replace(/\D/g, ""))} placeholder="000000" /><span style={{ fontSize: 11, color: "#7A7167" }}>The code expires in 10 minutes.</span></label>}
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3"><label style={label}>PASSWORD<input className="vivit-input" type="password" required minLength={8} maxLength={128} autoComplete="new-password" value={form.password} onChange={(event) => update("password", event.target.value)} placeholder="8+ characters" /></label><label style={label}>CONFIRM PASSWORD<input className="vivit-input" type="password" required minLength={8} maxLength={128} autoComplete="new-password" value={form.confirmPassword} onChange={(event) => update("confirmPassword", event.target.value)} placeholder="Repeat password" /></label></div>
            ...
            <label style={label}>WHY DO YOU NEED THIS ACCESS? (OPTIONAL)<textarea className="vivit-input" rows={3} maxLength={500} value={form.approvalNote} onChange={(event) => update("approvalNote", event.target.value)} placeholder="Team, department, or a short note for the admin" style={{ resize: "vertical" }} /></label>
