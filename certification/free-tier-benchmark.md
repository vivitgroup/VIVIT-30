...
- Re-running the failed certification job restores the latest checkpoint for the same code SHA and continues from the next pending case.
...
- Any code SHA change starts a fresh checkpoint scope so results from an older implementation cannot certify newer code.
