...
export default function VivitoVoiceRuntime(){
 const [inputHost,setInputHost]=useState<HTMLElement|null>(null),[barHost,setBarHost]=useState<HTMLElement|null>(null),[voiceState,setVoiceState]=useState<VoiceState>("idle"),[seconds,setSeconds]=useState(0),[note,setNote]=useState(""),[voiceReplies,setVoiceReplies]=useState(()=>typeof window!=="undefined"&&window.localStorage.getItem("vivitoVoiceReplies")==="on"),[speaking,setSpeaking]=useState(false);
 const recorderRef=useRef<MediaRecorder|null>(null),streamRef=useRef<MediaStream|null>(null),chunksRef=useRef<Blob[]>([]),timerRef=useRef<number|null>(null),audioRef=useRef<HTMLAudioElement|null>(null),audioUrlRef=useRef<string|null>(null),voiceRepliesRef=useRef(voiceReplies),knownMessages=useRef(new WeakSet<HTMLElement>()),panelRef=useRef<HTMLElement|null>(null);

 const stopTimer=useCallback(()=>{if(timerRef.current!==null){window.clearInterval(timerRef.current);timerRef.current=null}},[]);
 const releaseMic=useCallback(()=>{stopTimer();streamRef.current?.getTracks().forEach(track=>track.stop());streamRef.current=null;recorderRef.current=null},[stopTimer]);
 const stopAudio=useCallback(()=>{audioRef.current?.pause();audioRef.current=null;if(audioUrlRef.current){URL.revokeObjectURL(audioUrlRef.current);audioUrlRef.current=null}setSpeaking(false)},[]);
 ...
 },[releaseMic,stopAudio,transcribe,voiceState]);
 ...
 useEffect(()=>()=>{if(timerRef.current!==null)window.clearInterval(timerRef.current);streamRef.current?.getTracks().forEach(track=>track.stop());audioRef.current?.pause();if(audioUrlRef.current)URL.revokeObjectURL(audioUrlRef.current)},[]);
 useEffect(()=>{
  const sync=()=>{
   ...
   else messages.forEach(message=>{const isNew=!knownMessages.current.has(message);knownMessages.current.add(message);if(!message.querySelector(".va-voice-listen")){const button=document.createElement("button");button.type="button";button.className="va-voice-listen";button.setAttribute("aria-label","Listen to this VIVITO reply");button.textContent="◖ Listen";button.onclick=event=>{event.stopPropagation();void speak(messageText(message))};message.appendChild(button)}if(isNew&&voiceRepliesRef.current){const text=messageText(message);if(text)void speak(text)}})
  };
...
const CSS=`
.va-voice-mic-host{display:contents}.va-voice-mic{border:1px solid rgba(255,255,255,.1)!important;width:44px!important;height:44px!important;min-width:44px;border-radius:13px!important;background:rgba(255,255,255,.07)!important;color:#fff!important;display:grid;place-items:center;cursor:pointer;transition:.18s ease}.va-voice-mic:hover{background:rgba(255,255,255,.12)!important}.va-voice-mic:focus-visible,.va-voice-bar>button:focus-visible,.va-voice-listen:focus-visible{outline:2px solid rgba(224,179,79,.9)!important;outline-offset:2px}.va-voice-mic:disabled{opacity:.45;cursor:wait}.va-mic-glyph{position:relative;width:10px;height:15px;border:1.8px solid currentColor;border-radius:8px;display:block}.va-mic-glyph:before{content:"";position:absolute;left:50%;bottom:-6px;width:14px;height:8px;border:1.8px solid currentColor;border-top:0;border-radius:0 0 9px 9px;transform:translateX(-50%)}.va-mic-glyph:after{content:"";position:absolute;left:50%;bottom:-9px;width:1.8px;height:4px;background:currentColor;transform:translateX(-50%)}.va-voice-mic.recording{background:rgba(225,70,83,.16)!important;border-color:rgba(255,100,113,.5)!important;box-shadow:0 0 0 5px rgba(225,70,83,.08)}.va-voice-mic.recording .va-mic-glyph{animation:vivitoMicPulse 1s ease-in-out infinite}
.va-voice-bar-host{position:relative;padding:0 10px 7px}.va-voice-bar{min-height:52px;border:1px solid rgba(255,255,255,.08);background:rgba(255,255,255,.03);border-radius:13px;padding:6px 8px;display:flex;align-items:center;justify-content:space-between;gap:8px}.va-voice-state{min-width:0;display:flex;align-items:center;gap:8px}.va-voice-state>i{width:7px;height:7px;border-radius:50%;background:#7d7181;flex:none}.va-voice-state>i.active{background:#ff6574;box-shadow:0 0 0 5px rgba(255,101,116,.09);animation:vivitoVoiceDot 1s infinite}.va-voice-state div{min-width:0}.va-voice-state b{display:block;font-size:9px;letter-spacing:.1em;color:#e6dce9}.va-voice-state span{display:block;margin-top:3px;max-width:245px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;color:#9b8e9e;font-size:9px}.va-voice-bar>button{min-height:44px;border:1px solid rgba(255,255,255,.09);border-radius:10px;background:rgba(255,255,255,.045);color:#cfc5d2;font-size:9px;font-weight:800;letter-spacing:.03em;padding:0 10px;cursor:pointer;white-space:nowrap}.va-voice-bar>button.on{background:rgba(111,46,121,.24);border-color:rgba(190,118,205,.28);color:#fff}.va-voice-bar>button em{font-style:normal;font-size:9px;margin-left:3px;opacity:.65}.va-voice-listen{display:flex!important;align-items:center!important;justify-content:center!important;margin-top:8px!important;padding:0 10px!important;width:max-content!important;min-width:74px!important;min-height:44px!important;border:1px solid rgba(255,255,255,.08)!important;border-radius:10px!important;background:rgba(255,255,255,.035)!important;color:#aaa0ad!important;font:inherit!important;font-size:9px!important;font-weight:800!important;cursor:pointer!important;white-space:normal!important}.va-voice-listen:hover{color:#fff!important;background:rgba(255,255,255,.07)!important}
@keyframes vivitoMicPulse{50%{transform:scale(.9);opacity:.72}}@keyframes vivitoVoiceDot{50%{opacity:.35}}@media(max-width:560px){.va-voice-bar{min-height:56px}.va-voice-state span{max-width:39vw}.va-voice-bar>button{min-width:112px}.va-voice-mic{width:44px!important;height:44px!important;min-width:44px}.va-input>button{width:44px!important;height:44px!important;min-width:44px}}
