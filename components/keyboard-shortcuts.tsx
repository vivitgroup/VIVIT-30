...
const readTheme=():Theme=>localStorage.getItem("vivit-theme")==="light"?"light":"dark";
...
export function KeyboardShortcutsModal() {
  ...
  useEffect(() => {
    ...
    const handler = (e: KeyboardEvent) => {
      ...
      if (match) { router.push(match.href); buf = ""; }
    };
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"8px"}}>
          {SHORTCUTS.filter(s=>s.href!=="search").map(s=>(
            <div key={s.keys.join("")} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 12px",borderRadius:"8px",background:"rgba(255,255,255,0.03)"}}>
              ...
              <div style={{display:"flex",gap:"3px"}}>{s.keys.map(k=><kbd key={k} style={{background:"rgba(0,119,182,0.15)",border:"1px solid rgba(0,119,182,0.25)",borderRadius:"4px",padding:"1px 6px",fontFamily:"monospace",fontSize:"11px",color:"#00B4D8"}}>{k}</kbd>)}</div>
            </div>
          ))}
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 12px",borderRadius:"8px",background:"rgba(255,255,255,0.03)"}}>
            ...
            <div style={{display:"flex",gap:"3px"}}>{["T","T"].map((k,i)=><kbd key={i} style={{background:"rgba(0,119,182,0.15)",border:"1px solid rgba(0,119,182,0.25)",borderRadius:"4px",padding:"1px 6px",fontFamily:"monospace",fontSize:"11px",color:"#00B4D8"}}>{k}</kbd>)}</div>
          </div>
        </div>
