...
const SECTIONS:Section[]=[
 {label:"MAIN",items:[
  ...
  {icon:"🔗",label:"Integrations",href:"/dashboard/media/integrations",roles:["SUPER_ADMIN","MEDIA_BUYER"]},
...
export function Sidebar({role,userName}:{role:string;userName:string}){
 ...
 const visibleItems=visible.flatMap(s=>s.items);
 const activeHref=visibleItems.filter(i=>pathname===i.href||(i.href!=="/dashboard"&&pathname.startsWith(i.href+"/"))).sort((a,b)=>b.href.length-a.href.length)[0]?.href;
 ...
 return <aside className={`app-sidebar${collapsed?" collapsed":""}`}>
  <div className="sidebar-logo" style={{justifyContent:collapsed?"center":"flex-start"}}><Image src="/vivit-mark.png" alt="VIVIT" width={collapsed?38:58} height={50} style={{objectFit:"contain",maxWidth:collapsed?38:58,transition:"all .2s ease"}} priority/>{!collapsed&&<div className="sidebar-brand-copy"><strong>VIVIT</strong><span>Marketing ERP</span></div>}</div>
