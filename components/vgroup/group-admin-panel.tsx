...
export function GroupAdminPanel(){
  const[employees,setEmployees]=useState<any[]>([]);const[notifications,setNotifications]=useState<N[]>([]);const[audit,setAudit]=useState<Audit[]>([]);const[archive,setArchive]=useState<Archived[]>([]);const[error,setError]=useState("");
  const load=async()=>{try{const[e,n,a,r]=await Promise.all([fetch("/api/vgroup/admin/employees",{cache:"no-store"}),fetch("/api/vgroup/notifications",{cache:"no-store"}),fetch("/api/vgroup/admin/audit",{cache:"no-store"}),fetch("/api/vgroup/admin/archive",{cache:"no-store"})]);if(!e.ok||!n.ok||!a.ok||!r.ok)throw new Error("admin_load_failed");setEmployees((await e.json()).employees||[]);setNotifications((await n.json()).notifications||[]);setAudit((await a.json()).audit||[]);setArchive((await r.json()).archive||[])}catch(x){setError(x instanceof Error?x.message:"admin_load_failed")}};
  useEffect(()=>{load()},[]);
  ...
  return <div style={{display:"grid",gap:30}}>
    <section><h2>Employees <small style={{opacity:.6}}>({employees.length})</small></h2><div style={{display:"grid",gap:8}}>{employees.slice(0,50).map((e:any)=><div key={e.id} style={{...box,display:"flex",justifyContent:"space-between",gap:12,flexWrap:"wrap"}}><span><b>{e.full_name||e.email}</b><br/><small>{e.email} · {e.business_unit} · {(e.roles||[]).join(", ")||"No role"}</small></span><span style={{display:"flex",gap:6,alignItems:"center",flexWrap:"wrap"}}><b>{e.status||"active"}</b>{e.status!=="suspended"?<button onClick={()=>employeeStatus(e.id,"suspended")}>Suspend</button>:<button onClick={()=>employeeStatus(e.id,"active")}>Reactivate</button>}{e.status!=="archived"?<button onClick={()=>employeeStatus(e.id,"archived")}>Archive</button>:<button onClick={()=>restoreEmployee(e.id)}>Restore</button>}</span></div>)}</div></section>
