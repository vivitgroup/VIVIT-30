...
export function GroupVivitoChat({initialWorkspace="group"}:{initialWorkspace?:string}){
  ...
  async function transcribe(blob:Blob){
    if(!blob.size)return;setVoiceBusy(true);try{const form=new FormData();form.set("op","transcribe");form.set("file",new File([blob],"voice.webm",{type:blob.type||"audio/webm"}));const r=await fetch("/api/vgroup/vivito/voice",{method:"POST",body:form,cache:"no-store"});const d=await r.json().catch(()=>({}));if(!r.ok||!d.text)throw new Error(String(d.error||"Voice transcription failed"));await send(String(d.text))}catch(error){setMessages(m=>[...m,{who:"vivito",text:error instanceof Error?error.message:"Voice transcription failed."}])}finally{setVoiceBusy(false)}}
  ...
  async function speak(text:string){setVoiceBusy(true);try{const r=await fetch("/api/vgroup/vivito/voice",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({op:"synthesize",text}),cache:"no-store"});if(!r.ok){const d=await r.json().catch(()=>({}));throw new Error(String(d.error||"Voice synthesis failed"))}const blob=await r.blob(),url=URL.createObjectURL(blob),audio=new Audio(url);audio.onended=()=>URL.revokeObjectURL(url);audio.onerror=()=>URL.revokeObjectURL(url);await audio.play()}catch(error){setMessages(m=>[...m,{who:"vivito",text:error instanceof Error?error.message:"Voice synthesis failed."}])}finally{setVoiceBusy(false)}}
  ...
  return <section style={{border:"1px solid #263244",borderRadius:28,background:"rgba(12,17,25,.92)",overflow:"hidden",boxShadow:"0 30px 90px rgba(0,0,0,.34)"}}>
    ...
    <div style={{minHeight:360,maxHeight:560,overflowY:'auto',padding:18,display:'flex',flexDirection:'column',gap:14}}>{messages.length===0?<div style={{margin:'auto',textAlign:'center',color:'#75869a'}}><div style={{fontSize:30,fontWeight:900,color:'#f8fbff'}}>Ask VIVITO anything.</div><p style={{maxWidth:520,lineHeight:1.6}}>Type or speak. Voice is transcribed into the same governed VIVITO brain, so ERP scope and action permissions do not change.</p></div>:messages.map((m,i)=><div key={i} style={{alignSelf:m.who==='you'?'flex-end':'flex-start',maxWidth:'86%'}}><div style={{fontSize:10,letterSpacing:'.14em',color:'#72849b',marginBottom:5}}>{m.who==='you'?'YOU':'VIVITO'}</div><div style={{whiteSpace:'pre-wrap',lineHeight:1.65,padding:'13px 15px',borderRadius:16,background:m.who==='you'?'linear-gradient(135deg,#17345f,#244d87)':'#151c27',border:'1px solid #2a3648'}}>{m.text}</div>{m.who==='vivito'&&<button onClick={()=>speak(m.text)} disabled={voiceBusy} style={{marginTop:6,border:'1px solid #2a3648',borderRadius:10,background:'#0c131d',color:'#8bd3ff',padding:'6px 9px',fontSize:11,cursor:'pointer'}}>🔊 Listen</button>}{m.modelId&&<div style={{fontSize:10,color:'#5f7289',marginTop:5}}>model: {m.modelId}</div>}</div>)}{busy&&<div style={{color:'#8bd3ff'}}>VIVITO is thinking…</div>}{voiceBusy&&<div style={{color:'#8bd3ff'}}>Voice engine is working…</div>}</div>
