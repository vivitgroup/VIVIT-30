...
export function JsonMutationForm({endpoint,title,submitLabel="Save",fields,tone="group"}:{endpoint:string;title:string;submitLabel?:string;fields:Field[];tone?:"hospitality"|"tech"|"group"}){
 ...
 return <form onSubmit={submit} style={{padding:20,border:"1px solid #d7dde7",borderRadius:20,background:"#fff",display:"grid",gap:12,color:"#111827",minWidth:0,maxWidth:"100%",boxSizing:"border-box"}}><h3 style={{margin:0,fontSize:18}}>{title}</h3><div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(min(180px,100%),1fr))",gap:10,minWidth:0}}>{fields.map(f=><label key={f.name} style={{display:"grid",gap:6,fontSize:12,fontWeight:800,minWidth:0}}>{f.label}<input name={f.name} type={f.type||"text"} required={f.required} placeholder={f.placeholder} defaultValue={f.defaultValue} readOnly={f.readOnly} style={{width:"100%",minWidth:0,maxWidth:"100%",boxSizing:"border-box",padding:"12px",borderRadius:12,border:"1px solid #cfd6df",background:f.readOnly?"#f3f4f6":"#fff",color:"#111827",opacity:f.readOnly?0.8:1,fontSize:16}}/></label>)}</div><button disabled={busy} style={{justifySelf:"start",padding:"11px 16px",borderRadius:999,border:0,background:accent,color:"#071018",fontWeight:900,cursor:"pointer",opacity:busy?0.6:1}}>{busy?"Saving…":submitLabel}</button>{message?<div role="status" style={{fontSize:13,color:ok?"#166534":"#b42318",fontWeight:700,overflowWrap:"anywhere"}}>{message}</div>:null}</form>
}
