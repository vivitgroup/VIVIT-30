...
export default async function PropertyGallery({propertyId}:{propertyId:string}){
  ...
  return <section style={{maxWidth:1240,margin:"20px auto 0",padding:"0 22px",fontFamily:"Inter,system-ui,sans-serif"}}><article style={{padding:18,borderRadius:22,border:"1px solid rgba(214,173,91,.28)",background:"rgba(214,173,91,.035)"}}><div style={{display:"flex",justifyContent:"space-between",gap:14,alignItems:"baseline",flexWrap:"wrap"}}><div><div style={{fontSize:11,fontWeight:900,letterSpacing:".14em",color:"#D6AD5B"}}>PROPERTY GALLERY · {property.name.toUpperCase()}</div><h2 style={{margin:"6px 0 0"}}>Apartment images</h2></div><Link href="/group/hospitality/properties" style={{fontWeight:900,textDecoration:"none",color:"#D6AD5B"}}>Add images →</Link></div>{rendered.length?<div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(180px,1fr))",gap:10,marginTop:14}}>{rendered.map(image=><figure key={image.id} style={{margin:0}}><div style={{height:150,borderRadius:16,background:image.url?`url(${image.url}) center/cover no-repeat`:`linear-gradient(145deg,#183246,#0C1B2A)`,border:image.is_cover?"1px solid rgba(214,173,91,.75)":"1px solid rgba(214,173,91,.2)"}}/><figcaption style={{fontSize:11,color:"#C7B894",marginTop:5}}>{image.is_cover?"Cover · ":""}{image.caption||image.alt_text||image.file_name}</figcaption><PropertyImageActions propertyId={propertyId} imageId={image.id} isCover={image.is_cover}/></figure>)}</div>:<div style={{marginTop:12,padding:"14px 0",color:"#C7B894",lineHeight:1.7}}><strong style={{color:"#F7E3AA"}}>No property images are stored yet.</strong><div>{channel?`Airbnb calendar is ${channel.status}, but the current Airbnb connection is iCal availability sync; iCal does not carry listing photos. Upload images once and every Hospitality surface will read them from the same property_images source.`:"Add the apartment cover/gallery once and every Hospitality surface will read from the same property_images source."}</div></div>}</article></section>;
}
