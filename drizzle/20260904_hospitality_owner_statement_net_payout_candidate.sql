...
begin
  ...
  limit 1;
  if v_bu is null then raise exception 'hospitality_business_unit_unavailable'; end if;

  if not exists (
    select 1 from hospitality.owners o
    where o.id=p_owner_id
      and o.business_unit_id=v_bu
      and o.archived_at is null
  ) then
    raise exception 'owner_not_found_or_out_of_scope';
  end if;
  ...
  join hospitality.properties p
    on p.id=r.property_id
   and p.business_unit_id=v_bu
  where p.owner_id=p_owner_id
    and p.archived_at is null
    and r.business_unit_id=v_bu
  ...
  where i.business_unit_id=v_bu
    and i.archived_at is null
    and i.status in ('approved','paid')
    and (
      i.owner_id=p_owner_id
      or i.property_id in (
        select p2.id from hospitality.properties p2
        where p2.owner_id=p_owner_id
          and p2.business_unit_id=v_bu
