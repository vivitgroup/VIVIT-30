...
export async function createTaskRoleSafe(formData:FormData){
  ...
  const taskId=await db.transaction(async tx=>{
    ...
    await tx.insert(auditLogs).values({workspaceId,userId,action:"task_created",entity:"CreativeTask",entityId:task.id,newValues:JSON.stringify({title,clientId,type,priority,createdByRole:role,dualOperator:dual})});
    return task.id;
  });
