...
export async function updateClient(clientId:string,formData:FormData){
  const session=await auth();const access=await requireClientAccess(session,clientId,true),workspaceId=access.workspaceId,updateRole=roleOf(session);
  const [existingClient]=await db.select({accountManagerId:clients.accountManagerId,mediaBuyerId:clients.mediaBuyerId}).from(clients).where(and(eq(clients.id,clientId),eq(clients.workspaceId,workspaceId))).limit(1);if(!existingClient)throw new Error("Client not found");
  ...
  if(updateRole==="SUPER_ADMIN"){
    ...
    if(nextAm){const [am]=await db.select({id:users.id}).from(users).where(and(eq(users.id,nextAm),eq(users.workspaceId,workspaceId),eq(users.role,"ACCOUNT_MANAGER"),eq(users.isActive,true))).limit(1);if(!am)throw new Error("Invalid account manager")}
    if(nextMb){const [mb]=await db.select({id:users.id}).from(users).where(and(eq(users.id,nextMb),eq(users.workspaceId,workspaceId),eq(users.role,"MEDIA_BUYER"),eq(users.isActive,true))).limit(1);if(!mb)throw new Error("Invalid media buyer")}
  }
  await db.update(clients).set({companyName:String(formData.get("companyName")||""),industry:String(formData.get("industry")||"")||null,website:String(formData.get("website")||"")||null,monthlyRetainer:parseFloat(String(formData.get("monthlyRetainer")||""))||0,mediaBudget:parseFloat(String(formData.get("mediaBudget")||""))||0,contractValue:parseFloat(String(formData.get("contractValue")||""))||0,accountManagerId:nextAm,mediaBuyerId:nextMb,metaAdsLink:String(formData.get("metaAdsLink")||"")||null,tiktokAdsLink:String(formData.get("tiktokAdsLink")||"")||null,snapchatAdsLink:String(formData.get("snapchatAdsLink")||"")||null,googleAdsLink:String(formData.get("googleAdsLink")||"")||null,internalNotes:String(formData.get("internalNotes")||"")||null,updatedAt:new Date()}).where(and(eq(clients.id,clientId),eq(clients.workspaceId,workspaceId)));
...
export async function createTask(formData:FormData){
  ...
  const [task]=await db.insert(creativeTasks).values({workspaceId,title,clientId,type,brief,tov:tov||null,priority,status:"PENDING",assignedToId,deadline:deadlineDate,caption,createdById:session.user.id!}).returning();
...
export async function updateTaskStatus(taskId:string,status:string,revisionNotes?:string){
  ...
  const creatorTransitions:Record<string,string[]>={PENDING:["IN_PROGRESS"],IN_PROGRESS:["REVIEW"],REVISION:["IN_PROGRESS"]},managerTransitions:Record<string,string[]>={PENDING:["IN_PROGRESS"],IN_PROGRESS:["REVIEW"],REVISION:["IN_PROGRESS"],REVIEW:["APPROVED","REVISION","REJECTED"],APPROVED:["COMPLETED"]};
  ...
  const [currentTask]=await db.select({revisionCount:creativeTasks.revisionCount}).from(creativeTasks).where(and(eq(creativeTasks.id,taskId),eq(creativeTasks.workspaceId,workspaceId)));
  const [task]=await db.update(creativeTasks).set({status,completedAt:["APPROVED","COMPLETED"].includes(status)?new Date():undefined,revisionCount:status==="REVISION"?(currentTask?.revisionCount??0)+1:undefined,revisionNotes:status==="REVISION"?(revisionNotes??null):undefined,updatedAt:new Date()}).where(and(eq(creativeTasks.id,taskId),eq(creativeTasks.workspaceId,workspaceId))).returning();
  if(task?.assignedToId&&task.assignedToId!==session.user.id){const msgs:Record<string,string>={APPROVED:`✅ "${task.title}" was approved!`,REJECTED:`❌ "${task.title}" was rejected.`,REVISION:`↩ "${task.title}" needs revision.`,IN_PROGRESS:`🎨 Creator started working on "${task.title}"`,REVIEW:`👀 "${task.title}" is ready for review`};await db.insert(notifications).values({userId:task.assignedToId,type:"GENERAL",title:msgs[status]??`Task updated: ${status}`,message:`Status: ${status}`,link:`/dashboard/creative/${taskId}`})}
  await db.insert(auditLogs).values({workspaceId,userId:session.user.id!,action:`task_${status.toLowerCase()}`,entity:"CreativeTask",entityId:taskId,newValues:JSON.stringify({status})});revalidatePath(`/dashboard/creative/${taskId}`);revalidatePath("/dashboard/creative");
}

export async function submitTaskFile(taskId:string,fileName:string,fileUrl:string,notes:string){
  const session=await auth();requireRole(session,["CREATOR","SUPER_ADMIN","ACCOUNT_MANAGER"]);const {workspaceId,task:taskBefore}=await taskForAccess(session,taskId),role=String(session.user.role),isManager=["SUPER_ADMIN","ACCOUNT_MANAGER"].includes(role),allowedStatuses=role==="SUPER_ADMIN"?["PENDING","IN_PROGRESS","REVIEW","REVISION","APPROVED","COMPLETED"]:role==="ACCOUNT_MANAGER"?["PENDING","IN_PROGRESS","REVIEW","REVISION"]:["IN_PROGRESS","REVISION"];
  if((!isManager&&taskBefore.assignedToId!==session.user.id)||!allowedStatuses.includes(taskBefore.status))throw new Error("Forbidden");if(isManager)await requireClientAccess(session,taskBefore.clientId,true);if(!validateUrl(String(fileUrl||"")))throw new Error("A valid uploaded file URL is required");
  const [task]=await db.update(creativeTasks).set({status:"REVIEW",fileUrl:fileUrl||null,updatedAt:new Date()}).where(and(eq(creativeTasks.id,taskId),eq(creativeTasks.workspaceId,workspaceId))).returning();
  if(task){await db.insert(notifications).values({userId:task.createdById,type:"APPROVAL_REQUESTED",title:`📤 "${task.title}" submitted for review`,message:`${session.user.name} submitted. File: ${fileName}. ${notes}`,link:`/dashboard/creative/${taskId}`});const [client]=await db.select({userId:clients.userId}).from(clients).where(and(eq(clients.id,task.clientId),eq(clients.workspaceId,workspaceId)));if(client?.userId)await db.insert(notifications).values({userId:client.userId,type:"APPROVAL_REQUESTED",title:"🎨 New creative ready for review",message:`${task.title} is ready. Please review and approve.`,link:"/dashboard/portal"})}
  revalidatePath(`/dashboard/creative/${taskId}`);
}

export async function updateTaskCaption(taskId:string,caption:string){
  const session=await auth();if(!session?.user)throw new Error("Unauthorized");const {workspaceId,task}=await taskForAccess(session,taskId),role=roleOf(session);
  if(role==="CREATOR"){if(task.assignedToId!==session.user.id||!["PENDING","IN_PROGRESS","REVIEW","REVISION"].includes(task.status))throw new Error("This caption is locked.")}else{await requireClientAccess(session,task.clientId,true);if(role!=="SUPER_ADMIN"&&!["PENDING","IN_PROGRESS","REVIEW","REVISION"].includes(task.status))throw new Error("Approved or completed work is locked.")}
  await db.update(creativeTasks).set({caption,updatedAt:new Date()}).where(and(eq(creativeTasks.id,taskId),eq(creativeTasks.workspaceId,workspaceId)));revalidatePath(`/dashboard/creative/${taskId}`);
}
export async function markTaskPosted(taskId:string){
  const session=await auth();const {workspaceId,task}=await taskForAccess(session,taskId);await requireClientAccess(session,task.clientId,true);if(task.status!=="APPROVED")throw new Error("Only approved tasks can be marked posted");
  await db.update(creativeTasks).set({isPosted:true,postedAt:new Date(),status:"COMPLETED",updatedAt:new Date()}).where(and(eq(creativeTasks.id,taskId),eq(creativeTasks.workspaceId,workspaceId)));revalidatePath(`/dashboard/creative/${taskId}`);revalidatePath("/dashboard/creative");
}
