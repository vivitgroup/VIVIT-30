...
type ConnectionTokenInput=Pick<ConnectionRow,"id"|"accessTokenEncrypted"|"refreshTokenEncrypted"|"tokenExpiresAt"|"platform"|"adAccountId">;
...
export async function connectionAccessToken(connection:ConnectionTokenInput){if(!connection.accessTokenEncrypted)return"";const exp=connection.tokenExpiresAt?new Date(connection.tokenExpiresAt).getTime():0;if(!exp||exp>Date.now()+5*60_000)return decryptToken(connection.accessTokenEncrypted);const refreshToken=decryptToken(connection.refreshTokenEncrypted);if(!refreshToken)return decryptToken(connection.accessTokenEncrypted);const next=await refresh(connection.platform as OAuthPlatform,refreshToken);const values:Partial<typeof adPlatformConnections.$inferInsert>={accessTokenEncrypted:encryptToken(next.accessToken),refreshTokenEncrypted:encryptToken(next.refreshToken||refreshToken),tokenExpiresAt:next.expiresIn?new Date(Date.now()+next.expiresIn*1000):null,status:"CONNECTED",syncError:null,updatedAt:new Date()};await db.update(adPlatformConnections).set(values).where(eq(adPlatformConnections.id,connection.id));return next.accessToken;}
