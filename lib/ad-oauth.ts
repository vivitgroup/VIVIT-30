...
function encryptionSecret(){const dedicated=String(process.env.OAUTH_ENCRYPTION_KEY||"").trim();if(dedicated)return dedicated;if(process.env.NODE_ENV==="production")throw new Error("OAUTH_ENCRYPTION_KEY is required in production");return String(process.env.AUTH_SECRET||"").trim();}
...
export function decryptToken(value?:string|null){if(!value)return "";const parts=value.split(".");if(parts.length!==3)throw new Error("Invalid encrypted OAuth token");const [iv,tag,data]=parts;const decipher=crypto.createDecipheriv("aes-256-gcm",key(),Buffer.from(iv,"base64url"));decipher.setAuthTag(Buffer.from(tag,"base64url"));return Buffer.concat([decipher.update(Buffer.from(data,"base64url")),decipher.final()]).toString("utf8");}
