...
function getConfig(){
  ...
  return {secret,url,key,headers:{apikey:key,Authorization:`Bearer ${key}`}};
}
...
async function consumeNonce(claims:HandoffClaims):Promise<void>{
  const {url,key,headers}=getConfig();
  const nonceHash=createHash("sha256").update(claims.nonce).digest("hex");
  const response=await fetch(`${url}/rest/v1/group_handoff_nonces`,{
    method:"POST",
    headers:{...headers,"Content-Type":"application/json",Prefer:"return=minimal"},
    body:JSON.stringify({nonce_hash:nonceHash,group_user_id:claims.sub,email:claims.email,expires_at:new Date(claims.exp*1000).toISOString()}),
    cache:"no-store",
