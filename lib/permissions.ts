...
export const ROLE_PERMISSIONS: Record<string, Permission[]> = {
  ...
  MEDIA_BUYER:["view_reports","export_data","view_clients","view_media","edit_media","manage_budgets"],
  CREATOR:["view_tasks","edit_tasks"],
  ACCOUNTANT:["view_finance","create_invoices","edit_invoices","view_payroll","manage_payroll","view_reports","export_data","view_clients","create_clients","view_team","view_salaries","view_salary_recommendations","approve_salary_finance","view_payroll_lock","view_commissions","view_kpi_scores","view_agency_health","view_cfo_dashboard","view_resource_planning"],
  SALES:["view_sales","create_leads","edit_leads","view_proposals","create_proposals","view_reports"],
  CLIENT:["view_portal","approve_creatives","pay_invoices"],
};
export function hasPermission(role:string,permission:Permission,customPermissions?:Permission[]):boolean{if(customPermissions&&customPermissions.length>0)return customPermissions.includes(permission);return(ROLE_PERMISSIONS[role]??[]).includes(permission)}
export function hasAllPermissions(role:string,permissions:Permission[]):boolean{return permissions.every(p=>hasPermission(role,p))}
export function hasAnyPermission(role:string,permissions:Permission[]):boolean{return permissions.some(p=>hasPermission(role,p))}
export function homeFor(role:Role):string{switch(role){case Role.CLIENT:return "/dashboard/portal";case Role.CREATOR:return "/dashboard/creative";case Role.ACCOUNTANT:return "/dashboard/finance";case Role.MEDIA_BUYER:return "/dashboard/media/control-center";case Role.SALES:return "/dashboard/sales";case Role.ACCOUNT_MANAGER:return "/dashboard/clients";default:return "/apps"}}
...
{group:"Clients",icon:"🏢",permissions:[{key:"view_clients",label:"View Clients",desc:"See client list and details"},{key:"create_clients",label:"Create Clients",desc:"Add new clients"},{key:"edit_clients",label:"Edit Clients",desc:"Modify client information"},{key:"delete_clients",label:"Delete Clients",desc:"Remove clients (Admin only)"}]},
{group:"Creative Tasks",icon:"🎨",permissions:[{key:"view_tasks",label:"View Tasks",desc:"See task list"},{key:"create_tasks",label:"Create Tasks",desc:"Create new tasks with briefs"},{key:"edit_tasks",label:"Edit Tasks",desc:"Modify tasks and briefs"},{key:"delete_tasks",label:"Delete Tasks",desc:"Permanently remove tasks"},{key:"approve_tasks",label:"Approve Tasks",desc:"Approve/reject creative submissions"},{key:"assign_tasks",label:"Assign Tasks",desc:"Assign tasks to creators"}]},
