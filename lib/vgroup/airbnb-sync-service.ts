...
export type AirbnbSyncResult={ok:true;channelId:string;propertyId:string;propertyName:string;listing:string;events:number};
...
export async function syncAirbnbChannel(channelId:string):Promise<AirbnbSyncResult>{
  ...
  try{
    ...
    return {ok:true,channelId:channel.id,propertyId:channel.property_id,propertyName:channel.property_name,listing:channel.external_listing_id,events:events.length};
  }catch(error){
...
export async function syncAllAirbnbChannels(){
  ...
  const results:{channelId:string;ok:boolean;events?:number;listing?:string;propertyName?:string;error?:string}[]=[];
  for(const row of channels){
    try{const result=await syncAirbnbChannel(row.id);results.push({channelId:row.id,ok:true,events:result.events,listing:result.listing,propertyName:result.propertyName})}
  ...
  return {total:channels.length,synced:results.filter(item=>item.ok).length,failed:results.filter(item=>!item.ok).length,results};
}
