...
export async function syncAirbnbChannel(channelId:string,userId?:string):Promise<AirbnbSyncResult>{
  if(!uuid.test(channelId))throw new Error("Invalid Airbnb channel id");
  if(userId&&!uuid.test(userId))throw new Error("Invalid sync actor id");
  ...
  const [channel]=await sql<ChannelRow[]>`
    ...
    where c.id=${channelId}::uuid and c.channel='airbnb' and c.status<>'disabled' and p.archived_at is null limit 1`;
  ...
  try{
    ...
    const result=await sql.begin(async tx=>{
      for(const event of events){
        await tx`insert into hospitality.calendar_blocks(business_unit_id,property_id,channel_connection_id,external_uid,summary,starts_on,ends_on,source,last_seen_at,archived_at)
      ...
      await tx`update hospitality.calendar_blocks set archived_at=now(),updated_at=now() where channel_connection_id=${channel.id}::uuid and archived_at is null and last_seen_at<${syncStarted}::timestamptz and ends_on>=current_date`;
      const [conflictRow]=await tx<{conflicts:number}[]>`
...
export async function syncAllAirbnbChannels(userId?:string){
  if(userId&&!uuid.test(userId))throw new Error("Invalid sync actor id");
  const sql=getVGroupSql();
  const channels=await sql<{id:string}[]>`
    ...
    join hospitality.properties p on p.id=c.property_id
    join vgroup.business_units bu on bu.id=p.business_unit_id and bu.code='hospitality' and bu.status='active'
    where c.channel='airbnb' and c.status<>'disabled' and p.archived_at is null and c.token_ref is not null and btrim(c.token_ref)<>''
  ...
  for(const row of channels){
    try{const result=await syncAirbnbChannel(row.id,userId);results.push({channelId:row.id,ok:true,events:result.events,conflicts:result.conflicts,listing:result.listing,propertyName:result.propertyName})}
