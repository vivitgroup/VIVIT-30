...
export const VGROUP_MARKETING_BASE_SHA="9817ec42750b17104c5292eb2ec4d02358b53290";
export const VGROUP_PINNED_MARKETING_SHA="9817ec42750b17104c5292eb2ec4d02358b53290";

export type MarketingIntegrationState={enabled:boolean;candidateSha:string;certified:boolean};
type MarketingHandoffClaims={v:1;sub:string;email:string;name:string;business_unit:"marketing";iat:number;exp:number;nonce:string};
...
export function createMarketingHandoffAssertion(session:VGroupSession){
  ...
  const claims:MarketingHandoffClaims={v:1,sub:session.userId,email:normalizeEmail(session.email),name:session.fullName,business_unit:"marketing",iat:now,exp:now+45,nonce:randomUUID()};
  const payload=Buffer.from(JSON.stringify(claims),"utf8").toString("base64url");
  const signature=createHmac("sha256",secret).update(payload).digest("base64url");
  return {assertion:`${payload}.${signature}`,expiresAt:claims.exp};
}
...
export function createMarketingHandoff(session:VGroupSession){
  ...
  return {endpoint:target.toString(),...createMarketingHandoffAssertion(session)};
}
