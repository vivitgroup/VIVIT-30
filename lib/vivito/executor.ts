import {db,clients,contacts,creativeTasks,notifications,auditLogs,companyExpenses,financeRecords,paymentRecords,workspaces,sql} from "@/lib/db";
...
type ClientLookupRow={id:string;company_name:string;workspace_id?:string;is_active?:boolean;archived_at?:unknown;account_manager_id?:string|null;media_buyer_id?:string|null;user_id?:string|null};
...
const objectRecord=(value:unknown):ActionArgs=>value&&typeof value==="object"&&!Array.isArray(value)?value as ActionArgs:{};
...
async function findClient(refRaw:unknown,role:string,userId:string,includeArchived=false){
 ...
 let found=rows<ClientLookupRow>(await db.execute(sql`select id,company_name,workspace_id,is_active,archived_at,account_manager_id,media_buyer_id,user_id from clients where workspace_id=${tenantId()} and (id=${ref} or lower(company_name)=lower(${ref})) ${includeArchived?sql``:sql`and is_active=true and archived_at is null`} limit 3`));
 if(!found.length){found=rows<ClientLookupRow>(await db.execute(sql`select id,company_name,workspace_id,is_active,archived_at,account_manager_id,media_buyer_id,user_id from clients where workspace_id=${tenantId()} and company_name ilike ${`%${ref}%`} ${includeArchived?sql``:sql`and is_active=true and archived_at is null`} order by company_name limit 6`));}
...
async function findUser(refRaw:unknown,expectedRoles:string[]){
 ...
 let found=rows<UserLookupRow>(await db.execute(sql`select id,name,role from users where workspace_id=${tenantId()} and is_active=true and role in (${sql.join(expectedRoles.map(x=>sql`${x}`),sql`,`)}) and (id=${ref} or lower(name)=lower(${ref})) limit 3`));
 if(!found.length)found=rows<UserLookupRow>(await db.execute(sql`select id,name,role from users where workspace_id=${tenantId()} and is_active=true and role in (${sql.join(expectedRoles.map(x=>sql`${x}`),sql`,`)}) and name ilike ${`%${ref}%`} order by name limit 6`));
...
async function findTask(args:ActionArgs,role:string,userId:string,includeArchived=false){
 const taskId=clean(args.taskId,100),taskTitle=clean(args.taskTitle||args.title,180),clientName=clean(args.clientName,180);
 let client:ClientLookupRow|null=null;if(clientName)client=await findClient(clientName,role,userId,true);
 let q:SQL;
 if(taskId)q=sql`select t.id,t.title,t.client_id,t.archived_at,t.assigned_to_id,c.company_name,c.account_manager_id,c.is_active as client_active from creative_tasks t join clients c on c.id=t.client_id where t.workspace_id=${tenantId()} and c.workspace_id=${tenantId()} and t.id=${taskId} limit 2`;
...
async function createClientAction(args:ActionArgs,role:string,userId:string){
 ...
 const duplicate=await db.select({id:clients.id}).from(clients).where(and(eq(clients.workspaceId,tenantId()),ilike(clients.companyName,companyName))).limit(1);if(duplicate[0])throw new VivitoActionError("A client with this company name already exists.",409,{clientId:duplicate[0].id});
 ...
 const [client]=await db.insert(clients).values({workspaceId:tenantId(),companyName,industry:clean(args.industry,100)||null,website:clean(args.website,500)||null,monthlyRetainer:nonNegative(args.monthlyRetainer),mediaBudget:canSetupMarketing?nonNegative(args.mediaBudget):0,contractValue:nonNegative(args.contractValue),accountManagerId,mediaBuyerId,contractStart,contractEnd,internalNotes:canSetupMarketing?(clean(args.internalNotes,2000)||null):null}).returning();
 if(clean(args.contactName,160))await db.insert(contacts).values({clientId:client.id,name:clean(args.contactName,160),email:clean(args.contactEmail,254)||null,phone:clean(args.contactPhone,60)||null,whatsapp:clean(args.contactPhone,60)||null,isPrimary:true});
...
async function clientLifecycle(op:VivitoActionOp,args:ActionArgs,role:string,userId:string){
 ...
 const [deps]=rows<ClientDependencyRow>(await db.execute(sql`select (select count(*)::int from creative_tasks where client_id=${c.id}) tasks,(select count(*)::int from file_documents where client_id=${c.id}) files,(select count(*)::int from calendar_events where client_id=${c.id}) calendar,(select count(*)::int from finance_records where client_id=${c.id}) finance,(select count(*)::int from ad_campaigns where client_id=${c.id}) campaigns,(select count(*)::int from contacts where client_id=${c.id}) contacts,(select count(*)::int from sales_leads where client_id=${c.id}) converted_leads`));
 ...
 await db.execute(sql`delete from clients where id=${c.id} and workspace_id=${tenantId()}`);await audit(userId,"vivito_client_deleted","clients",c.id,{companyName:c.company_name});return{success:true,action:op,entityId:c.id,message:`Client ${c.company_name} permanently deleted.`};
}

async function createTaskAction(args:ActionArgs,role:string,userId:string){
 ...
 const [task]=await db.insert(creativeTasks).values({workspaceId:tenantId(),title,clientId:client.id,type:type as CreativeTaskInsert["type"],brief,priority:priority as CreativeTaskInsert["priority"],status:"PENDING",assignedToId:creatorId,deadline,createdById:userId}).returning();
 if(creatorId)await db.insert(notifications).values({userId:creatorId,type:"TASK_ASSIGNED",title:`VIVITO assigned: ${title}`,message:`${client.company_name} · ${deadline.toLocaleDateString("en-EG")}`,link:`/dashboard/creative/${task.id}`,priority:priority==="URGENT"?"high":"normal"});
...
async function taskLifecycle(op:VivitoActionOp,args:unknown,role:string,userId:string){
 const t=await findTask(objectRecord(args),role,userId,true);
 ...
 if(role!=="SUPER_ADMIN")throw new VivitoActionError("Only Super Admin can permanently delete a task.",403);if(!t.archived_at)throw new VivitoActionError("Archive the task before permanent deletion.",409);const [deps]=rows<TaskDependencyRow>(await db.execute(sql`select (select count(*)::int from file_documents where task_id=${t.id}) files,(select count(*)::int from calendar_events where task_id=${t.id}) calendar,(select count(*)::int from task_comments where task_id=${t.id}) comments`));if(Number(deps?.files||0)+Number(deps?.calendar||0)+Number(deps?.comments||0)>0)throw new VivitoActionError("Task has linked files, comments, or calendar items.",409,{dependencies:deps});await db.execute(sql`delete from creative_tasks where id=${t.id} and workspace_id=${tenantId()}`);await audit(userId,"vivito_task_deleted","creative_tasks",t.id,{title:t.title});return{success:true,action:op,entityId:t.id,message:`Task ${t.title} permanently deleted.`};
}

async function logExpenseAction(args:ActionArgs,userId:string){
 const category=clean(args.category||"Other",40),description=clean(args.description,500),value=amount(args.amount);if(!EXPENSES.has(category)||!description)throw new VivitoActionError("Expense category and description are required.");const now=args.date?parsedDate(args.date):new Date(),dateOnly=now.toISOString().slice(0,10);let id="";await db.transaction(async tx=>{const [row]=await tx.insert(companyExpenses).values({workspaceId:tenantId(),category,description,amount:value,date:now,approvedBy:userId}).returning();id=row.id;await tx.execute(sql`insert into financial_ledger_entries(id,workspace_id,entry_date,direction,entry_type,category,counterparty,description,amount,source_sheet,source_ref,source_key,approved_by,created_at) values(gen_random_uuid()::text,${tenantId()},${dateOnly}::date,'OUT','VIVITO_EXPENSE',${category},${description},${description},${value},'VIVITO',${row.id},${`vivito:expense:${row.id}`},${userId},now())`);await tx.insert(auditLogs).values({workspaceId:tenantId(),userId,action:"vivito_expense_logged",entity:"company_expenses",entityId:row.id,newValues:JSON.stringify({category,description,amount:value,ledger:true})})});return{success:true,action:"log_expense",entityId:id,message:`Expense ${value.toLocaleString("en-EG")} logged under ${category}.`,link:"/dashboard/finance"};
}

async function recordPaymentAction(args:ActionArgs,role:string,userId:string){
...
export async function executeVivitoAction(op:VivitoActionOp,args:unknown,role:string,userId:string,workspaceId:string){if(!workspaceId)throw new VivitoActionError("Workspace unavailable.",403);return workspaceContext.run(workspaceId,async()=>{
 ...
 if(op==="create_client")return createClientAction(input,role,userId);
 if(["archive_client","restore_client","delete_client"].includes(op))return clientLifecycle(op,input,role,userId);
 if(op==="create_task")return createTaskAction(input,role,userId);
 if(["archive_task","restore_task","delete_task"].includes(op))return taskLifecycle(op,input,role,userId);
 if(op==="log_expense")return logExpenseAction(input,userId);
 if(op==="record_payment")return recordPaymentAction(input,role,userId);
 if(op==="create_invoice")return createInvoiceAction(input,role,userId);
 if(op==="attach_file")return attachFileAction(input,role,userId);
 if(op==="remind_me")return reminderAction(input,userId);
