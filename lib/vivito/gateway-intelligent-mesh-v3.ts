...
export async function generateViaGatewayIntelligentMesh(prompt:string,system:string,token:string,options:GenerateOptions={}){
 ...
 for(const modelId of candidates){
  ...
  try{
   ...
   const servedModel=String(data.model||modelId);if(!eligible.has(servedModel)){markFailure(modelId,0,"gateway-returned-model-outside-verified-free-pool");errors.push(`${modelId}:gateway-returned-model-outside-verified-free-pool`);continue}
