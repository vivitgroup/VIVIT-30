...
async function token(explicit?:string){const direct=String(explicit||process.env.GROQ_API_KEY||"").trim();if(direct)return direct;try{const jar=await cookies();return unsealGroqSecret(jar.get(GROQ_KEY_COOKIE)?.value||"")}catch{return ""}}
...
export function groqFreeConfigured(){return true}
...
export async function discoverGroqFreeModels(force=false,apiKey?:string):Promise<GroqFreeModelId[]>{
  const key=await token(apiKey);if(!key)throw new Error("groq-free-not-configured");
...
export async function generateViaGroqFreeMesh(prompt:string,system:string,options:Options={}){
  const key=await token(options.apiKey);if(!key)throw new Error("groq-free-not-configured");const live=await discoverGroqFreeModels(false,key);
