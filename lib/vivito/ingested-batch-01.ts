export type VivitoIngestedLesson={domain:string;source:string;title:string;published?:string;sourceType:'official'|'expert';confidence:'high'|'medium';lessons:string[]};
...
export const VIVITO_INGESTED_BATCH_01:VivitoIngestedLesson[]=[
...
export const VIVITO_INGESTED_BATCH_01_CONTEXT=VIVITO_INGESTED_BATCH_01.map(x=>`## ${x.domain} — ${x.source}: ${x.title}\n${x.lessons.map(v=>`- ${v}`).join('\n')}`).join('\n\n');
