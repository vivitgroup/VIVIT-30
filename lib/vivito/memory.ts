...
export async function forgetVivitoMemory(queryRaw:string,userId:string,role:string,authorizedClientIds:string[],workspaceId:string){if(!workspaceId)throw new Error("Workspace unavailable.");
 const query=clean(queryRaw,500).toLowerCase();if(!query)throw new Error("Tell VIVITO what memory to forget.");const memories=await loadVivitoMemories(userId,role,authorizedClientIds,workspaceId,200),matches=memories.filter(m=>m.text.toLowerCase().includes(query)||query.includes(m.text.toLowerCase())).slice(0,20);if(!matches.length)return{forgotten:0,ids:[] as string[]};for(const m of matches){if(m.scopeType==="WORKSPACE"&&role!=="SUPER_ADMIN")continue;await db.insert(auditLogs).values({workspaceId,userId,action:"vivito_memory_forgotten",entity:"vivito_memory",entityId:m.id,newValues:JSON.stringify({memoryId:m.id,forgottenBy:userId,forgottenAt:new Date().toISOString()})})}return{forgotten:matches.length,ids:matches.map(m=>m.id)};
}
...
export async function loadVivitoMemories(userId:string,role:string,authorizedClientIds:string[],workspaceId:string,limit=60):Promise<VivitoMemory[]>{if(!workspaceId)throw new Error("Workspace unavailable.");
 const raw=Array.from(await db.execute(sql`select action,entity_id,new_values,user_id,created_at from audit_logs where workspace_id=${workspaceId} and entity='vivito_memory' and action in ('vivito_memory_saved','vivito_memory_forgotten') order by created_at desc limit 500`)) as MemoryAuditRow[];
 const seen=new Set<string>(),out:VivitoMemory[]=[];for(const row of raw){const id=String(row.entity_id||"");if(!id||seen.has(id))continue;seen.add(id);if(row.action==="vivito_memory_forgotten")continue;let p:Record<string,unknown>={};try{p=objectRecord(typeof row.new_values==="string"?JSON.parse(row.new_values):row.new_values)}catch{continue}const scope=String(p.scopeType||"USER") as VivitoMemoryScope,scopeId=p.scopeId?String(p.scopeId):null,createdBy=String(p.createdBy||row.user_id||"");const visible=scope==="WORKSPACE"||(scope==="USER"&&createdBy===userId)||(scope==="CLIENT"&&!!scopeId&&authorizedClientIds.includes(scopeId));if(!visible)continue;out.push({id,kind:String(p.kind||"RULE") as VivitoMemoryKind,scopeType:scope,scopeId,text:clean(p.text),createdBy,createdAt:String(p.createdAt||row.created_at||"")});if(out.length>=limit)break}return out;
}
