...
export function isVivitoMemoryVisible(memory:Pick<VivitoMemory,"scopeType"|"scopeId"|"createdBy">,userId:string,authorizedClientIds:string[]){return memory.scopeType==="WORKSPACE"||(memory.scopeType==="USER"&&memory.createdBy===userId)||(memory.scopeType==="CLIENT"&&!!memory.scopeId&&authorizedClientIds.includes(memory.scopeId))}
export function forgettableVivitoMemories(matches:VivitoMemory[],role:string){return matches.filter(m=>m.scopeType!=="WORKSPACE"||role==="SUPER_ADMIN")}
...
export async function forgetVivitoMemory(queryRaw:string,userId:string,role:string,authorizedClientIds:string[],workspaceId:string){if(!workspaceId)throw new Error("Workspace unavailable.");
 const query=clean(queryRaw,500).toLowerCase();if(!query)throw new Error("Tell VIVITO what memory to forget.");const memories=await loadVivitoMemories(userId,role,authorizedClientIds,workspaceId,200),matches=memories.filter(m=>m.text.toLowerCase().includes(query)||query.includes(m.text.toLowerCase())).slice(0,20);if(!matches.length)return{forgotten:0,ids:[] as string[]};const forgettable=forgettableVivitoMemories(matches,role),forgottenIds:string[]=[];for(const m of forgettable){const value:typeof auditLogs.$inferInsert={workspaceId,userId,action:"vivito_memory_forgotten",entity:"vivito_memory",entityId:m.id,newValues:JSON.stringify({memoryId:m.id,forgottenBy:userId,forgottenAt:new Date().toISOString()})};await db.insert(auditLogs).values(value);forgottenIds.push(m.id)}return{forgotten:forgottenIds.length,ids:forgottenIds};
}
