...
export async function executeVivitoPlanRuntime(input:{
 ...
 for(let i=0;i<steps.length;i++){
  ...
  const previous=Array.from(await db.execute(sql`select new_values from audit_logs where workspace_id=${workspaceId} and user_id=${userId} and action='vivito_plan_step_executed' and new_values::jsonb->>'stepKey'=${stepKey} order by created_at desc limit 1`)) as any[];
  ...
  try{
   ...
   await db.insert(auditLogs).values({workspaceId,userId,action:"vivito_plan_step_executed",entity:"vivito_plan",entityId:requestId,newValues:JSON.stringify({requestId,stepKey,stepIndex:i,op:step.op,summary:step.summary||"",result:combined,approval:decisions[i]?.approval||null})} as any);
  }catch(error){
   ...
   await db.insert(auditLogs).values({workspaceId,userId,action:"vivito_plan_stopped",entity:"vivito_plan",entityId:requestId,newValues:JSON.stringify({requestId,stepIndex:i,op:step.op,args:step.args,message,completedSteps:results.length})} as any);
 ...
 await db.insert(auditLogs).values({workspaceId,userId,action:"vivito_plan_executed",entity:"vivito_plan",entityId:requestId,newValues:JSON.stringify({requestId,stepCount:results.length,results,duplicateSteps})} as any);
