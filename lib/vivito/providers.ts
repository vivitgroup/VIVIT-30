...
function transparentAdvisorFailure(prompt:string,attempted:VivitoProviderName[],errors:string[],started:number):VivitoGeneration{
  ...
  else text=arabic?"VIVITO شغال بوضع الاستمرارية الآمن. بيانات الـERP المصرح بها ما زالت متاحة داخليًا حسب صلاحياتك، لكن لن أعرض السياق الداخلي أو JSON أو حقول النظام الخام. مزودات الـAI الخارجية المجانية غير متاحة مؤقتًا؛ أعد المحاولة أو اكتب أمر ERP مباشر.":"VIVITO is online in secure continuity mode. Authorized ERP context remains available internally for your role, but internal context, JSON, and raw system fields are never exposed. External free AI providers are temporarily unavailable; retry or send a direct ERP command.";
  console.warn("VIVITO secure continuity fallback",{attempted,errors:errors.slice(-6)});
