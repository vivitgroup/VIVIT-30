...
function transparentAdvisorFailure(prompt:string,attempted:VivitoProviderName[],errors:string[],started:number):VivitoGeneration{
  const question=requestFromPrompt(prompt),arabic=/[\u0600-\u06ff]/.test(question),greeting=/^(?:hi|hello|hey|good\s+(?:morning|afternoon|evening)|مرحبا|مرحباً|اهلا|أهلا|السلام عليكم)[\s!.?،]*$/i.test(question.trim());
  const contextHit=prompt.match(/ERP LIVE CONTEXT:\s*([\s\S]*?)(?:\n\n(?:AUTHORIZED|TRUSTED|ATTACHMENTS|DIRECTORY)|$)/i),context=String(contextHit?.[1]||"").replace(/\s+/g," ").trim().slice(0,900);
  let text:string;
  ...
  else if(context)text=arabic?`مزودات الـAI الخارجية المجانية غير متاحة مؤقتًا، لكن VIVITO ما زال شغال بوضع الاستمرارية على بيانات الـERP المصرح بها. السياق الحالي: ${context}`:`The external free AI providers are temporarily unavailable, but VIVITO is still online in continuity mode using your authorized ERP context. Current context: ${context}`;
  else text=arabic?"VIVITO شغال بوضع الاستمرارية. مزودات الـAI الخارجية المجانية غير متاحة مؤقتًا، لكن أوامر الـERP والإجراءات المسموح بها ما زالت متاحة. اكتب الطلب بشكل مباشر.":"VIVITO is online in continuity mode. External free AI providers are temporarily unavailable, but authorized ERP actions remain available. Send the request directly.";
  console.warn("VIVITO contextual continuity fallback",{attempted,errors:errors.slice(-6),hasContext:Boolean(context)});
  return{text,provider:"local",attempted:[...attempted,"local"],errors,latencyMs:Date.now()-started,modelId:"vivito-context-continuity-v2"};
}
