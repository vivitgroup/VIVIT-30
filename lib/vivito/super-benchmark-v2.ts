...
for(let i=0;i<70;i++){const s=marketingSeeds[i%marketingSeeds.length] as any;out.push(C(`MKT-${v(i+1)}`,"marketing",`${s[0]} Scenario variant ${i+1}: give the decision, evidence needed, economics, experiment and stop/scale rule.`,s[1],[],1));}
...
for(let i=0;i<50;i++){const s=salesSeeds[i%salesSeeds.length] as any;out.push(C(`SAL-${v(i+1)}`,"sales",`${s[0]} Variant ${i+1}: respond as Sales Director with owner, next action and measurement.`,s[1],[],1));}
...
for(let i=0;i<50;i++){const s=businessSeeds[i%businessSeeds.length] as any;out.push(C(`BUS-${v(i+1)}`,"business",`${s[0]} Variant ${i+1}: include P&L/cash implications, assumptions, downside and decision trigger.`,s[1],[],1));}
...
for(let i=0;i<40;i++){const s=crossSeeds[i%crossSeeds.length] as any;out.push(C(`XFN-${v(i+1)}`,"crossFunctional",`${s[0]} Variant ${i+1}: synthesize one decision across Marketing, Sales, Finance and Operations.`,s[1],[],1));}
...
for(let i=0;i<30;i++){const s=researchSeeds[i%researchSeeds.length] as any;out.push(C(`RES-${v(i+1)}`,"research",`${s[0]} Variant ${i+1}: provide an evidence plan and do not invent live facts.`,s[1],["fabricated citation","made-up source"],1));}
...
for(let i=0;i<20;i++){const s=adversarialSeeds[i%adversarialSeeds.length] as any;out.push(C(`ADV-${v(i+1)}`,"adversarial",`${s[0]} Variant ${i+1}.`,s[1],s[2],1.4));}
