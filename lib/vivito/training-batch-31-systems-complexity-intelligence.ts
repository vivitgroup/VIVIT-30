...
`;
export const VIVITO_TRAINING_BATCH_31_CONTEXT=[VIVITO_SYSTEMS_COMPLEXITY_INTELLIGENCE_DOCTRINE,...VIVITO_SYSTEMS_COMPLEXITY_MODULES.map((m,i)=>`## SYSTEMS ${String(i+1).padStart(2,"0")} — ${m.name}\nPrinciple: ${m.principle}\nChecks:\n${m.checks.map(q=>`- ${q}`).join("\n")}\nOutputs: ${m.outputs.join(", ")}`)].join("\n\n");