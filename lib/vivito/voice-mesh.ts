...
import {GROQ_KEY_COOKIE,unsealGroqSecret} from "./groq-user-key";

export type VivitoVoiceProvider="voder"|"vox"|"audio-cpp"|"groq";
...
async function groqKey(){const direct=String(process.env.GROQ_API_KEY||"").trim();if(direct)return direct;try{const jar=await cookies();return unsealGroqSecret(jar.get(GROQ_KEY_COOKIE)?.value||"")}catch{return ""}}
const configuredProviders=async():Promise<ProviderConfig[]>=>{
 const g=await groqKey();const providers:ProviderConfig[]=[
  {provider:"voder",url:trimUrl(process.env.VIVITO_VODER_URL||""),token:String(process.env.VIVITO_VODER_TOKEN||"").trim()||undefined},
  {provider:"groq",url:GROQ_BASE,token:g||undefined},
  {provider:"vox",url:trimUrl(process.env.VIVITO_VOX_URL||"")},
  {provider:"audio-cpp",url:trimUrl(process.env.VIVITO_AUDIOCPP_URL||"")},
 ];
 return providers.filter(x=>Boolean(x.url)&&!(x.provider==="groq"&&!x.token));
};
...
export async function vivitoVoiceConfigured(){return (await configuredProviders()).length>0}

export async function vivitoVoiceStatus():Promise<VoiceProviderStatus[]>{
 const g=await groqKey();const all:[VivitoVoiceProvider,string,string?][]=[
  ["voder",trimUrl(process.env.VIVITO_VODER_URL||""),String(process.env.VIVITO_VODER_TOKEN||"").trim()||undefined],
  ["groq",g?GROQ_BASE:"",g||undefined],
 ...
 return Promise.all(all.map(async([provider,url,token])=>{
  ...
  try{const endpoint=provider==="groq"?`${url}/models`:`${url}/health`;const r=await fetch(endpoint,{headers:token?{Authorization:`Bearer ${token}`}:{},cache:"no-store",signal:timeout(3500)});return{provider,configured:true,healthy:r.ok,url,detail:r.ok?"ready":`http-${r.status}`}}catch{return{provider,configured:true,healthy:false,url,detail:"unreachable"}}
 }));
}
...
async function transcribeOpenAICompatible(cfg:ProviderConfig,audio:Uint8Array,mimeType:string,language?:string):Promise<VoiceTranscript>{
 const started=Date.now(),form=new FormData();const groq=cfg.provider==="groq";const model=groq?"whisper-large-v3-turbo":String(process.env.VIVITO_AUDIOCPP_ASR_MODEL||"qwen3-asr").trim();form.set("model",model);if(language)form.set("language",language);form.set("response_format","json");form.set("file",new Blob([Buffer.from(audio)],{type:mimeType||"audio/wav"}),"input.wav");
 const r=await fetch(`${cfg.url}/audio/transcriptions`,{method:"POST",headers:headers(cfg),body:form,cache:"no-store",signal:timeout(30000)});const raw=await r.text();if(!r.ok)throw new Error(`${cfg.provider}-transcribe-${r.status}`);
 let text=raw;try{const d=JSON.parse(raw) as {text?:unknown};text=String(d.text||"").trim()}catch{}if(!text.trim())throw new Error(`${cfg.provider}-empty-transcript`);return{text:text.trim(),provider:cfg.provider,modelId:model,durationMs:Date.now()-started};
}
...
async function synthOpenAICompatible(cfg:ProviderConfig,text:string,voice?:string):Promise<VoiceSynthesis>{
 const started=Date.now(),groq=cfg.provider==="groq",arabic=/[\u0600-\u06ff]/.test(text),model=groq?(arabic?"canopylabs/orpheus-arabic-saudi":"canopylabs/orpheus-v1-english"):String(process.env.VIVITO_AUDIOCPP_TTS_MODEL||"pocket-tts").trim();if(groq&&text.length>200)throw new Error("groq-tts-text-over-200");const body:Record<string,unknown>={model,input:text,response_format:"wav"};body.voice=voice||(groq?(arabic?"noura":"hannah"):undefined);
 const r=await fetch(`${cfg.url}/audio/speech`,{method:"POST",headers:headers(cfg,{"Content-Type":"application/json"}),body:JSON.stringify(body),cache:"no-store",signal:timeout(30000)});if(!r.ok)throw new Error(`${cfg.provider}-synthesize-${r.status}`);const audio=new Uint8Array(await r.arrayBuffer());if(!audio.byteLength)throw new Error(`${cfg.provider}-empty-audio`);return{audio,contentType:r.headers.get("content-type")||"audio/wav",provider:cfg.provider,modelId:model,durationMs:Date.now()-started};
}
export async function synthesizeWithVivitoVoice(text:string,voice?:string):Promise<VoiceSynthesis>{const clean=String(text||"").trim().slice(0,6000);if(!clean)throw new Error("voice-text-required");const errors:string[]=[];for(const cfg of await configuredProviders()){try{return cfg.provider==="audio-cpp"||cfg.provider==="groq"?await synthOpenAICompatible(cfg,clean,voice):await synthVoderOrVox(cfg,clean,voice)}catch(error){errors.push(`${cfg.provider}:${error instanceof Error?error.message:"failed"}`)}}throw new Error(`vivito-voice-synthesis-unavailable:${errors.join("|")||"no-provider"}`)}
