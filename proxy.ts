...
export default auth((req) => {
  ...
  if (publicExact.includes(pathname) || publicPrefixes.some((prefix) => pathname.startsWith(prefix))) {
  ...
  if (!session) {
    if (pathname.startsWith("/api/")) {
      return secHeaders(NextResponse.json({ error: "Unauthorized" }, { status: 401 }));
    }
  ...
  if (pathname === "/dashboard") {
    const homes: Record<string, string> = {
      ...
      CREATOR: "/dashboard/creative",
      ACCOUNTANT: "/dashboard/finance",
      MEDIA_BUYER: "/dashboard/media/control-center",
      SALES: "/dashboard/sales",
      ACCOUNT_MANAGER: "/dashboard/clients",
    };
  ...
  const pageAccess: [string, string[]][] = [
    ...
    ["/dashboard/activity", ["SUPER_ADMIN"]],
    ["/dashboard/kpis", ["SUPER_ADMIN"]],
    ["/dashboard/billing", ["SUPER_ADMIN"]],
    ["/dashboard/referrals", ["SUPER_ADMIN"]],
    ["/dashboard/saas-analytics", ["SUPER_ADMIN"]],
    ["/dashboard/revenue-attribution", ["SUPER_ADMIN"]],
    ["/dashboard/nps", ["SUPER_ADMIN"]],
    ...
    ["/dashboard/contracts", ["SUPER_ADMIN", "ACCOUNTANT"]],
    ["/dashboard/finance", ["SUPER_ADMIN", "ACCOUNTANT"]],
    ["/dashboard/forecast", ["SUPER_ADMIN", "ACCOUNTANT"]],
    ["/dashboard/ltv", ["SUPER_ADMIN", "ACCOUNTANT"]],
  ...
  if (pathname.startsWith("/dashboard")) {
    ...
    if (rule && !rule[1].includes(role)) {
      return NextResponse.redirect(new URL(role === "CLIENT" ? "/dashboard/portal" : "/dashboard", req.url));
    }
  }
  ...
  if (role === "CLIENT") {
    ...
    if (pathname.startsWith("/dashboard") && !clientAllowed.some((prefix) => pathname.startsWith(prefix))) {
