...
async function main(){
 ...
 for(const target of targets){
  ...
  profiles.push({
   ...
   title:(result.profile.raw as unknown)?.title||null,
 ...
 let baseline:unknown=null;try{baseline=JSON.parse(fs.readFileSync('certification/competitive-live-baseline.json','utf8'))}catch{}
 ...
 let comparable=0;
 if(baseline?.profiles){for(const p of profiles){const old=baseline.profiles.find((x:unknown)=>x.profileUrl===p.profileUrl);if(old&&(['followers','totalPosts','totalLikes'] as const).some(k=>typeof old[k]==='number'&&typeof p[k]==='number'))comparable++;}}
