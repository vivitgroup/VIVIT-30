...
let cases=0;
const run=async(op:any,args:any)=>{const out=await executeVivitoOperatorAction(op,args,'SUPER_ADMIN',adminId);if(!out?.success)throw new Error(`${op} did not return success`);cases++;return out};
try{
 ...
 const users=Array.from(await db.execute(sql`select id,name,is_active from users where workspace_id='default' and email like 'cert-user-%@vivito.test' order by email` ) as any);
 const refs=Array.from(await db.execute(sql`select id,referrer_id from referrals where referred_email like 'referral-%@vivito.test'` ) as any);
 const audits=Array.from(await db.execute(sql`select id from audit_logs where workspace_id='default' and user_id=${adminId} and action like 'vivito_%'` ) as any);
 const allActive=users.length===8&&users.every((u:any)=>u.is_active===true);
 const referralOwner=refs.length===18&&refs.every((r:any)=>r.referrer_id===adminId);
 ...
 fs.writeFileSync('.vivito/execution-e2e.json',JSON.stringify(report,null,2));console.log(JSON.stringify(report,null,2));if(!passed)process.exit(1);
}catch(error){const report={passed:false,cases,error:error instanceof Error?error.message:String(error),database:"ephemeral-postgres",productionDataUsed:false};fs.writeFileSync('.vivito/execution-e2e.json',JSON.stringify(report,null,2));console.error(report);process.exit(1)}
