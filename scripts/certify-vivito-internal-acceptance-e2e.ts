...
async function main(){
  ...
  `)));
  const one=rows(await db.execute(sql`select id from vivito_autonomy_events where workspace_id=${W} and idempotency_key=${sameKey}`));
  ...
  const cross=rows(await db.execute(sql`select workspace_id from vivito_autonomy_events where idempotency_key=${sameKey} order by workspace_id`));
  assert.deepEqual(cross.map((r:any)=>r.workspace_id),[W,X],"same idempotency key must coexist across workspaces");
  ...
  const cp=rows(await db.execute(sql`select state,attempt,status from vivito_runtime_checkpoints where workspace_id=${W} and run_key='resume-proof'`))[0];
  ...
  const cps=rows(await db.execute(sql`select workspace_id from vivito_runtime_checkpoints where run_key='resume-proof' order by workspace_id`));
  assert.deepEqual(cps.map((r:any)=>r.workspace_id),[W,X],"checkpoint keys must remain tenant isolated");
  ...
  for(const h of [24,48,72]) await db.execute(sql`insert into vivito_outcome_checks(id,workspace_id,event_id,horizon_hours,due_at,status,baseline) values(${crypto.randomUUID()},${W},${executedId},${h},now()+(${h}::text||' hours')::interval,'PENDING','{}'::jsonb) on conflict(event_id,horizon_hours) do nothing`);
  const outcomes=rows(await db.execute(sql`select horizon_hours,status from vivito_outcome_checks where workspace_id=${W} and event_id=${executedId} order by horizon_hours`));
  assert.deepEqual(outcomes.map((r:any)=>Number(r.horizon_hours)),[24,48,72],"exactly the 24/48/72 horizons must be scheduled once");
  assert(outcomes.every((r:any)=>r.status==="PENDING"));
  ...
  assert(runtime.includes("await scheduleOutcomes(workspaceId,String(e.id)"),"retry success must call outcome scheduling");
  ...
  const due=rows(await db.execute(sql`select id from vivito_autonomy_events where workspace_id=${W} and status='RETRY_SCHEDULED' and next_retry_at<=now() and retry_count<max_retries`));
  assert(due.some((r:any)=>r.id===retryId),"persisted retry must be resumable after process restart");
  assert(runtime.includes("async function retryDue(actor:Actor)"),"production retry lifecycle must exist");
  assert(runtime.includes("retried=await retryDue(actor);const a=await collectAndAct(actor)"),"resume/retry must run before collecting new work");
  assert(runtime.includes("next>=Number(e.max_retries||3)"),"retry lifecycle must terminate at bounded max attempts");
  ...
  const rolled=rows(await db.execute(sql`select status,rolled_back_at,rolled_back_by,rollback_result from vivito_autonomy_events where id=${executedId} and workspace_id=${W}`))[0];
  ...
  assert(runtime.includes("if(String(e.status)!=='EXECUTED')")||runtime.includes('if(String(e.status)!=="EXECUTED")'),"rollback must only target executed events");
