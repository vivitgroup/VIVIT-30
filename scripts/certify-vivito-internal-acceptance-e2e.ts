...
const rows=<T extends Record<string,unknown>>(v:unknown)=>Array.from(v as Iterable<T>);
...
async function main(){
  ...
  `)));
  const one=rows<{id:string}>(await db.execute(sql`select id from vivito_autonomy_events where workspace_id=${W} and idempotency_key=${sameKey}`));
  ...
  const cross=rows<{workspace_id:string}>(await db.execute(sql`select workspace_id from vivito_autonomy_events where idempotency_key=${sameKey} order by workspace_id`));
  ...
  const cp=rows<{state:{phase:number;cursor?:string};attempt:number|string;status:string}>(await db.execute(sql`select state,attempt,status from vivito_runtime_checkpoints where workspace_id=${W} and run_key='resume-proof'`))[0];
  ...
  const cps=rows<{workspace_id:string}>(await db.execute(sql`select workspace_id from vivito_runtime_checkpoints where run_key='resume-proof' order by workspace_id`));
  ...
  for(const h of [24,48,72]) await db.execute(sql`insert into vivito_outcome_checks(id,workspace_id,event_id,horizon_hours,due_at,status,baseline) values(${crypto.randomUUID()},${W},${executedId},${h},now()+(${h}::text||' hours')::interval,'PENDING','{}'::jsonb) on conflict(event_id,horizon_hours) do nothing`);
  const outcomes=rows<{horizon_hours:number|string;status:string}>(await db.execute(sql`select horizon_hours,status from vivito_outcome_checks where workspace_id=${W} and event_id=${executedId} order by horizon_hours`));
  ...
  const due=rows<{id:string}>(await db.execute(sql`select id from vivito_autonomy_events where workspace_id=${W} and status='RETRY_SCHEDULED' and next_retry_at<=now() and retry_count<max_retries`));
  ...
  const rolled=rows<{status:string;rolled_back_at:unknown;rolled_back_by:string;rollback_result:{ok:boolean}}>(await db.execute(sql`select status,rolled_back_at,rolled_back_by,rollback_result from vivito_autonomy_events where id=${executedId} and workspace_id=${W}`))[0];
