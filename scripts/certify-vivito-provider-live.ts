...
async function main(){
 ...
 for(const [name,c] of Object.entries(cfg) as [ProviderName,(typeof cfg)[ProviderName]][]){
  ...
  }catch(error){providers[name]={passed:false,blocked:false,error:String((error as unknown)?.message||error).slice(0,500),campaignRef:digest(c.campaignId),accountRef:digest(c.accountId)};all=false;}
 }
