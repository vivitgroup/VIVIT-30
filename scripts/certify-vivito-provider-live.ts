...
const requested=(process.env.VIVITO_CERT_REQUIRED_PROVIDERS||"meta,snapchat")
 .split(",").map(v=>v.trim().toLowerCase()).filter(Boolean);
const requiredProviders=[...new Set(requested)].filter((v):v is ProviderName=>v in cfg);

async function main(){
 fs.mkdirSync('.vivito',{recursive:true});
 const providers:Partial<Record<ProviderName,unknown>>={};
 let all=requiredProviders.length>0;
 for(const name of requiredProviders){
  ...
  }catch(error){providers[name]={passed:false,blocked:false,error:String((error as Error)?.message||error).slice(0,500),campaignRef:digest(c.campaignId),accountRef:digest(c.accountId)};all=false;}
 }
 const report={passed:all,mode:"same-state-live-provider-certification",createdAt:new Date().toISOString(),requiredProviders,providers};
