...
async function prep(){
 ...
 await sql`insert into file_documents(id,workspace_id,uploaded_by,client_id,name,storage_path,mime_type,size_bytes,category) values(${alphaFile},${workspace},${creatorUser},${alphaClient},'ALPHA_PLAN.pdf','${workspace}/alpha/plan.pdf','application/pdf',100,'CONTENT_PLAN'),(${betaFile},${workspace},${creatorUser},${betaClient},'BETA_SECRET.pdf','${workspace}/beta/secret.pdf','application/pdf',100,'CONTENT_PLAN')`;
 await sql`insert into finance_records(id,workspace_id,client_id,month,year,total_revenue,paid,outstanding,invoice_number,invoice_status) values(gen_random_uuid()::text,${workspace},${alphaClient},9,2099,111,50,61,'ALPHA-INVOICE','SENT'),(gen_random_uuid()::text,${workspace},${betaClient},9,2099,999999,0,999999,'BETA-SECRET-INVOICE','SENT')`;
 await sql`insert into calendar_events(id,workspace_id,client_id,task_id,title,date,status) values(gen_random_uuid()::text,${workspace},${alphaClient},${alphaTask},'ALPHA_CALENDAR_EVENT',now()+interval '1 day','scheduled'),(gen_random_uuid()::text,${workspace},${betaClient},${betaTask},'BETA_PRIVATE_EVENT',now()+interval '1 day','scheduled'),(gen_random_uuid()::text,${workspace},${alphaClient},${alphaDeletedTask},'ALPHA_DELETED_EVENT',now()+interval '1 day','scheduled')`;
 await sql`insert into notifications(id,user_id,type,title,message,priority) values(gen_random_uuid()::text,${alphaUser},'INFO','ALPHA_NOTICE','Alpha only','normal'),(gen_random_uuid()::text,${betaUser},'INFO','BETA_PRIVATE_NOTICE','Beta only','normal')`;
 await sql`insert into task_comments(id,task_id,user_id,comment,is_internal) values(gen_random_uuid()::text,${alphaTask},${creatorUser},'ALPHA_PUBLIC_COMMENT',false),(gen_random_uuid()::text,${alphaTask},${creatorUser},'ALPHA_INTERNAL_SECRET',true)`;
}
...
const server=http.createServer((req,res)=>{
 const u=new URL(req.url||"/",`http://127.0.0.1:${mockPort}`);res.setHeader("content-type","application/json");
 if(u.pathname==="/rest/v1/users"){
  const emailFilter=u.searchParams.get("email")||"",idFilter=u.searchParams.get("id")||"";
  const email=emailFilter.startsWith("eq.")?decodeURIComponent(emailFilter.slice(3)):null,id=idFilter.startsWith("eq.")?decodeURIComponent(idFilter.slice(3)):null;
  const row=Object.values(users).find(x=>(email&&x.email===email)||(id&&x.id===id));if(!row){res.end("[]");return}
  const select=String(u.searchParams.get("select")||"").split(",").filter(Boolean),projected=select.length?Object.fromEntries(select.map(k=>[k,row[k]??null])):row;res.end(JSON.stringify([projected]));return;
 }
...
function absorbCookies(headers,jar){const list=typeof headers.getSetCookie==="function"?headers.getSetCookie():[headers.get("set-cookie")].filter(Boolean);for(const raw of list){const first=String(raw).split(";",1)[0],i=first.indexOf("=");if(i>0)jar.set(first.slice(0,i),first.slice(i+1));}}
const cookieHeader=jar=>[...jar].map(([k,v])=>`${k}=${v}`).join("; ");
async function request(path,{jar,method="GET",headers={},body}={}){const h={...headers};if(jar?.size)h.Cookie=cookieHeader(jar);const r=await fetch(base+path,{method,headers:h,body,redirect:"manual"});if(jar)absorbCookies(r.headers,jar);return r;}
async function login(who){const jar=new Map(),csrf=await request("/api/auth/csrf",{jar}),data=await csrf.json();const form=new URLSearchParams({csrfToken:String(data.csrfToken||""),email:users[who].email,password,callbackUrl:`${base}/dashboard/portal`});const r=await request("/api/auth/callback/credentials",{jar,method:"POST",headers:{"content-type":"application/x-www-form-urlencoded"},body:form});check(`${who} portal login succeeds`,[200,302,303].includes(r.status)&&[...jar.keys()].some(k=>k.includes("session-token")),`status=${r.status}`);return jar;}
...
try{
 ...
 const patchForeign=await request("/api/files",{jar:alpha,method:"PATCH",headers:{"content-type":"application/json"},body:JSON.stringify({id:betaFile,op:"edit",name:"stolen.pdf",category:"CONTENT_PLAN",clientId:betaClient})});check("Client cannot mutate another client's file",patchForeign.status===403,`status=${patchForeign.status}`);
}catch(error){check("Client Portal E2E runner completed",false,String(error?.stack||error));}
finally{try{server.close();}catch{}await cleanup();await sql.end({timeout:1});}
const failed=checks.filter(c=>!c.ok);for(const c of checks)console.log(`${c.ok?"PASS":"FAIL"}  ${c.name}${c.detail?` — ${c.detail}`:""}`);console.log(`\n${checks.length-failed.length}/${checks.length} client-portal checks passed.`);if(failed.length)process.exit(1);
