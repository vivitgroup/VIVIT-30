...
let buttons=0,anchors=0,links=0,forms=0,roleButtons=0,pages=0;
...
for(const file of files){
 ...
 const visit=(node:ts.Node)=>{
  if(ts.isJsxElement(node)||ts.isJsxSelfClosingElement(node)){
   ...
   if(t==="button"){
    buttons++;const type=staticText(attr(open,"type")).toLowerCase(),staticDisabled=staticText(attr(open,"disabled"))==="true";
    if(staticDisabled)findings.push({file:rel,line,kind:"static-disabled-button",detail:"Button is permanently disabled"});
    if(!spread&&!has(open,"onClick")&&!has(open,"formAction")&&!insideForm(open)&&type!=="submit"&&type!=="reset")findings.push({file:rel,line,kind:"dead-button",detail:"Button has no click/form action and is outside a form"});
   }
   if(t==="a"){
...
console.log(`\nInteractive inventory: ${files.length} TSX files · ${pages} pages · ${buttons} buttons · ${anchors} anchors · ${links} Links · ${forms} forms · ${roleButtons} role-buttons.`);
