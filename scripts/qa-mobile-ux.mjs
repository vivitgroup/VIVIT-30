...
const globals=read("app/globals.css"),mobileCss=read("app/dashboard/mobile-release.css"),microCss=read("app/dashboard/uiux-micro-fixes.css"),enterpriseCss=read("app/dashboard/enterprise-ui-baseline.css"),layout=read("app/dashboard/layout.tsx"),nav=read("components/layout/MobileNav.tsx"),header=read("components/layout/Header.tsx"),clientForm=globals;
...
check("Enterprise UI baseline is loaded last",layout.includes('import "./mobile-parity-v6.css";\nimport "./enterprise-ui-baseline.css";'));
check("Dashboard offers keyboard skip navigation",layout.includes('href="#dashboard-content"')&&layout.includes('className="skip-to-content"')&&layout.includes('id="dashboard-content"')&&layout.includes('tabIndex={-1}'));
check("Loading skeleton adapts from desktop to mobile",layout.includes('className="dashboard-skeleton-kpis"')&&layout.includes('className="dashboard-skeleton-main"')&&enterpriseCss.includes("grid-template-columns:repeat(2,minmax(0,1fr))")&&enterpriseCss.includes("grid-template-columns:1fr"));
check("Reduced-motion users get a motion-safe interface",enterpriseCss.includes("prefers-reduced-motion:reduce")&&enterpriseCss.includes("animation-duration:.01ms!important")&&enterpriseCss.includes("transition-duration:.01ms!important"));
check("Global keyboard focus remains visibly outlined",enterpriseCss.includes(":focus-visible")&&enterpriseCss.includes("outline:3px solid"));
...
check("Global Search traps keyboard focus inside its modal",header.includes("dialogRef")&&header.includes('e.key!=="Tab"')&&header.includes("document.activeElement===first")&&header.includes("document.activeElement===last"));
check("Global Search results announce updates accessibly",header.includes('aria-live="polite"')&&header.includes('aria-controls="global-search-results"'));
...
check("Mobile full menu closes with Escape",nav.includes('e.key==="Escape"')&&nav.includes("close()"));
check("Mobile full menu traps Tab focus while open",nav.includes("drawerRef")&&nav.includes('e.key!=="Tab"')&&nav.includes("document.activeElement===first")&&nav.includes("document.activeElement===last"));
check("Mobile full menu exposes dialog semantics",nav.includes('role="dialog"')&&nav.includes("aria-modal={open||undefined}"));
