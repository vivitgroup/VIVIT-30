...
pass("Lifecycle API has archive restore soft-delete and deleted restore",lifecycleApi.includes('"archive","restore","delete","restore_deleted"')&&lifecycleApi.includes("BUSINESS_LIFECYCLE_SPECS")&&lifecycleApi.includes("deleted_by=${userId}"));
pass("Delete Center is Super Admin only",lifecycleApi.includes('view==="deleted"&&!isAdmin')&&lifecycleApi.includes("Only Super Admin can view deleted records"));
pass("Lifecycle delete is audited and recoverable instead of hard delete",lifecycleApi.includes("softDelete:action==="+'"delete"')&&lifecycleApi.includes('action==="restore_deleted"')&&!/delete from clients where/i.test(lifecycleApi));
pass("Financial/history records have state-aware delete safeguards",lifecycleRegistry.includes("deleteAllowedStatuses")&&lifecycleRegistry.includes("Posted journal entries cannot be deleted")&&lifecycleRegistry.includes("Approved or paid expense claims are immutable"));
pass("Lifecycle ledger records actor for archive/delete",lifecycleApi.includes("lifecycle_events")&&lifecycleApi.includes("actor_user_id")&&lifecycleApi.includes("actor_name"));
pass("Archive and Delete Centers expose restore with actor tracking",archivePage.includes("Restore")&&archivePage.includes("Archived by")&&deletePage.includes("Restore")&&deletePage.includes("Deleted by"));
