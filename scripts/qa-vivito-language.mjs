...
$("General advisor outage stays explicit while continuity mode remains useful",providers.includes("transparentAdvisorFailure")&&(providers.includes("secure continuity fallback")||providers.includes("contextual continuity fallback"))&&providers.toLowerCase().includes("external free ai providers are temporarily unavailable")&&providers.includes("ERP LIVE CONTEXT")&&providers.includes("if(isGeneralAdvisorSystem(system))return transparentAdvisorFailure")&&!providers.includes('import {generateLocalAdvisorV2'));
