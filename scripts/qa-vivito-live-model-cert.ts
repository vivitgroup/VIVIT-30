...
const credential=()=>String(process.env.AI_GATEWAY_API_KEY||process.env.VERCEL_OIDC_TOKEN||"").trim();

async function main(){
  const token=credential();
  if(!token)throw new Error("AI_GATEWAY_API_KEY or VERCEL_OIDC_TOKEN is required for live model certification");
