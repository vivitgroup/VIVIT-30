...
async function main(){
 ...
 const groqModels=groqToken?await discoverGroqFreeModels(true,groqToken):[];
 ...
 assert.ok(catalogIdentities.size>=MIN_CERTIFIED,`need at least ${MIN_CERTIFIED} distinct underlying live free-tier text models across configured providers; found ${catalogIdentities.size}`);
 ...
 console.log(JSON.stringify({openRouterCatalogModels:openRouterModels.length,groqCatalogModels:groqModels.length,groqConfigured:Boolean(groqToken),distinctUnderlyingCandidateModels:catalogIdentities.size,certified:certifiedIdentities.size,providerPasses:{"openrouter-free":orPasses,"groq-free":groqPasses},failed},null,2));
 assert.ok(orPasses>0,"live certification requires at least one callable OpenRouter free model");assert.ok(certifiedIdentities.size>=MIN_CERTIFIED,`live certification requires >=${MIN_CERTIFIED} distinct underlying callable models across configured free providers; certified ${certifiedIdentities.size}`);
 console.log(`\nLIVE_CERTIFIED ${certifiedIdentities.size} distinct underlying models callable across configured free providers with exact pinned routes.`)
}
