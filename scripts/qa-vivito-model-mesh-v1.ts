...
async function main(){
  ...
  await check("registry parses, normalizes and deduplicates model IDs",()=>{const all=loadVivitoModelMesh();assert.equal(all.length,4);assert.equal(all[0].baseUrl,"https://mesh-a.example/v1")});
  ...
  globalThis.fetch=(async (input:RequestInfo|URL)=>{
    ...
    if(url.includes("mesh-a.example")){
  ...
  console.log(`\n${passed}/12 VIVITO Model Mesh V1 checks passed.`);
