...
const ids={
 super:"e2e-super-admin",accountManager:"e2e-account-manager",mediaBuyer:"e2e-media-buyer",creator:"e2e-creator",accountant:"e2e-accountant",sales:"e2e-sales",client:"e2e-client-user"
};
const accounts=[
...
async function main(){
 ...
 await db.insert(creativeTasks).values({id:"e2e-task",workspaceId,clientId:"e2e-client",assignedToId:ids.creator,title:"E2E Enterprise Creative",type:"REEL",status:"IN_PROGRESS",priority:"HIGH",brief:"Isolated browser certification task"}).onConflictDoNothing();
 await db.insert(salesLeads).values({id:"e2e-lead",workspaceId,companyName:"E2E Prospect",contactPerson:"Global Buyer",email:"buyer@example.test",stage:"QUALIFIED",estimatedValue:25000,probability:40,createdBy:ids.sales}).onConflictDoNothing();
