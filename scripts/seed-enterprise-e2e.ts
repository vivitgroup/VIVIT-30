...
async function main(){
 ...
 for(const project of browserProjects){
  await db.insert(creativeTasks).values({id:`e2e-task-${project}`,workspaceId,clientId:"e2e-client",createdById:ids.accountManager,assignedToId:ids.creator,title:`E2E Enterprise Creative ${project}`,type:"REEL",status:"IN_PROGRESS",priority:"HIGH",brief:`Isolated ${project} browser certification task`,deadline:new Date(Date.now()+86400000)}).onConflictDoNothing();
  await db.insert(clients).values({id:`e2e-fin-client-${project}`,workspaceId,companyName:`E2E Finance Client ${project}`,industry:"Professional Services",currency:"USD",monthlyRetainer:0,mediaBudget:0,contractValue:0,accountManagerId:ids.accountManager,mediaBuyerId:ids.mediaBuyer,isActive:true}).onConflictDoNothing();
 }
 ...
 console.log(`Seeded ${accounts.length} isolated E2E roles, ${browserProjects.length} creative workflows and ${browserProjects.length} finance setup clients in ${workspaceId}`);
}
