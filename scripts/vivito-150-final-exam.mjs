...
const TITLE_PREFIX=`Final${RUN_TAG} Task`;
const jar=new Map();
...
async function refreshProtection(){const res=await req(`${BASE}/?_vercel_share=${SHARE}`);if(res.status>=400)throw new Error(`preview-protection:${res.status}`)}
...
async function execute(proposal,i){const res=await api('/api/assistant/actions',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({op:proposal.op,args:proposal.args,confirm:true,requestId:`vivito-final-150-${RUN_TAG}-${i}`})}),body=await json(res);return{res,body}}
const questionSeeds=[
...
for(let i=1;i<=40;i++){
 await refreshProtection();
 const title=`${TITLE_PREFIX} ${String(i).padStart(2,'0')}`,deadline=`2026-09-${String(5+(i%19)).padStart(2,'0')}`,priority=i%9===0?'HIGH':'MEDIUM';
...
for(let i=41;i<=50;i++){
 await refreshProtection();
 const n=i-40,title=`${TITLE_PREFIX} ${String(n).padStart(2,'0')}`,priority=i%2?'HIGH':'LOW';
