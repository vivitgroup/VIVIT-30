...
async function preflight() {
  const rows = await sql`
    ...
    where table_schema = 'public'
      and table_name in ('users','workspaces','clients')
    order by table_name
  `;
...
async function readback() {
  ...
  const requiredTables = new Set([
    'vivito_autonomy_events','vivito_approval_events','vivito_decision_journal',
    'vivito_escalations','vivito_resource_usage','vivito_notification_dedupe',
    'vivito_security_events'
  ]);
  const seen = new Set(tables.map((r) => r.tablename));
  for (const name of requiredTables) {
    if (!seen.has(name)) throw new Error(`Production readback failed: missing public.${name}`);
  }
    ...
    where table_schema='public'
      and (
        (table_name='vivito_autonomy_events' and column_name in (
          'retry_count','next_retry_at','evidence_quality','decision_route','simulation',
          'policy_version','decision_version','learning_fingerprint','rollback_of_event_id',
          'rolled_back_at','rolled_back_by','rollback_result'
        ))
        or
        (table_name='vivito_backup_manifests' and column_name in (
  ...
  for (const col of [
    ...
    'vivito_autonomy_events.evidence_quality','vivito_autonomy_events.decision_route',
    'vivito_autonomy_events.simulation','vivito_autonomy_events.policy_version',
    'vivito_autonomy_events.decision_version','vivito_autonomy_events.learning_fingerprint',
    'vivito_autonomy_events.rollback_of_event_id','vivito_autonomy_events.rolled_back_at',
    'vivito_autonomy_events.rolled_back_by','vivito_autonomy_events.rollback_result',
    'vivito_backup_manifests.format_version','vivito_backup_manifests.verified_checksum',
    'vivito_backup_manifests.restore_record_counts','vivito_backup_manifests.verification_details'
  ]) {
  ...
  const indexes = await sql`
    ...
    where schemaname='public'
      and indexname in (
        'uq_vivito_event_workspace_idempotency','uq_vivito_checkpoint_run_key',
        'uq_vivito_backup_snapshot','uq_vivito_governance_scope'
      )
  `;
...
try {
  ...
  if (mode === 'APPLY') {
    ...
    console.log('CHECK mode: no schema or data changes were executed.');
  }
  await readback();
