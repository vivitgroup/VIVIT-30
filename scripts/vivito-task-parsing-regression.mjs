...
const BASE='https://vivit-erp-git-fix-vivito-action-planner-1a8cee-vivit-s-projects.vercel.app';
const SHARE='BnjNlh9Vvt9Qv7QAN2oUHjkgWghtxDyt';
...
async function execute(proposal,i){const res=await api('/api/assistant/actions',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({op:proposal.op,args:proposal.args,confirm:true,requestId:`task-parsing-regression-v3-${i}`})}),body=await json(res);return{res,body}}
...
for(let i=1;i<=40;i++){
 ...
 if(p.res.status!==200||p.body?.mode!=='action-proposal'||a?.op!=='create_task'||a?.args?.clientName!=='QA Exam Client'||a?.args?.title!==title||a?.args?.priority!==priority||a?.missingFields?.length){failures.push({i,phase:'proposal',status:p.res.status,mode:p.body?.mode,proposal:a});continue}
