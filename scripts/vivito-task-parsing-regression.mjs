...
const TITLE_PREFIX='BoundaryFix Task';
const jar=new Map();
...
async function execute(proposal,i){const res=await api('/api/assistant/actions',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({op:proposal.op,args:proposal.args,confirm:true,requestId:`task-parsing-regression-v8-${i}`})}),body=await json(res);return{res,body}}
